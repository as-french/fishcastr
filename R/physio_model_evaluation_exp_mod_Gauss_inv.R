#' Evaluate (inverted) exponentially modified Gaussian physiological response
#' curve
#'
#' @description This function produces a series of plots that can be used to
#'   evaluate the fit of an inverted exponentially modified Gaussian response
#'   curve to unimodal count data. Specifically, plots are intended for checking
#'   the consistency of curve fit (i.e., inter-decadal variability in a proxy
#'   for attainment of migration preparedness) among multiple decades (e.g.,
#'   fitting curves for pooled count data from windows 1990 - 2000, 2011 - 2010,
#'   20011 - 2020). Residual plots use the functionality of the DHARMa package
#'   (Hartig 2019). The intended implementation of the inverted exponentially
#'   modified Gaussian curve as a proxy for physiological preparedness for
#'   migration of diadromous fishes assumes that physiological preparedness for
#'   sea entry increases with degree days that reflect an increase in
#'   physiological condition for marine endurance (e.g., gill Na+/K+-ATPase
...
#'@export
physio_model_evaluation_exp_mod_Gauss_inv <- function(data_list,
                                                  species_count_lab,
                                                  x_variable_lab,
...
#  dnbinom <- stats::dnbinom
  ...
  plot(data_list[[1]][[x_variable_lab]][data_list[[1]][[species_count_lab]] != 0],
       ...
       yaxt = 'n')
  axis(side = 2,
       labels = c(1200,800,400,200,100,0,100,200,400,800,1200),
       at = c(-1200,-800,-400,-200,-100,0,100,200,400,800,1200))
...
#x = 1
  physio_model_eval <- plyr::llply(.data = 1:(length(data_list)), .fun = function(x){
    ...
    if(error_distribution_name == "negative_binomial"){
      formula_physio = formula(paste0(species_count_lab,"~dnbinom(mu = (100*c)*(((1/lamb)/2)*(exp(((1/lamb)/2)*(-2*mu_exmg + (1/lamb)*(sigma_exmg^2)-2*(-1)*",x_variable_lab,")))*pracma::erfc((-mu_exmg+(1/lamb)*(sigma_exmg^2) - (-1)*",x_variable_lab,")/(sqrt(2)*sigma_exmg))),size=1/sigma_nb)"))
    ...
    if(error_distribution_name == "generalised_Poisson"){
      formula_physio = formula(paste0(species_count_lab,"~dLGP(theta = (1 - (1-sqrt(1/phi)))*((100*c)*(((1/lamb)/2)*(exp(((1/lamb)/2)*(-2*mu_exmg + (1/lamb)*(sigma_exmg^2)-2*(-1)*",x_variable_lab,")))*pracma::erfc((-mu_exmg+(1/lamb)*(sigma_exmg^2) - (-1)*",x_variable_lab,")/(sqrt(2)*sigma_exmg)))), lambda=1-sqrt(1/phi))"))
...
#      formula_physio = formula(paste0(species_count_lab,"~dLGP(theta = (1 - (1-sqrt(1/phi)))*((100*c)*(((1/lamb)/2)*(exp(((1/lamb)/2)*(-2*mu_exmg + (1/lamb)*(sigma_exmg^2)-2*(-1)*",x_variable_lab,")))*pracma::erfc((-mu_exmg+(1/lamb)*(sigma_exmg^2) - (-1)*",x_variable_lab,")/(sqrt(2)*sigma_exmg)))), lambda=1-sqrt(1/phi))"))
    ...
    if(error_distribution_name == "quasi_Poisson"){
      formula_physio = formula(paste0(species_count_lab,"~dNBII(mu = (100*c)*(((1/lamb)/2)*(exp(((1/lamb)/2)*(-2*mu_exmg + (1/lamb)*(sigma_exmg^2)-2*(-1)*",x_variable_lab,")))*pracma::erfc((-mu_exmg+(1/lamb)*(sigma_exmg^2) - (-1)*",x_variable_lab,")/(sqrt(2)*sigma_exmg))),sigma=sigma_nb)"))
      starting_values = start_values
      lower_limits = lower_lims
    }
#    formula_physio
#    x = 1
 ...
 #   mod_physio_expmG
#
#    mod_physio_expmG1 = bbmle::mle2(formula_physio,
 #                                  start = starting_values,
  #                                 method = "Nelder-Mead",
   #                                control = list(maxit = 1000000000),
    #                               data=data_list[[x]])
    ...
    #mod_physio_expmG@call$data

    # View(mod_physio_expmG)
 ...
 #   suppressMessages(require(MASS, quietly = TRUE))
    ...
    dist = matrix(nrow = 5000, ncol = 1000)
    for (i in 1:1000) {
      dist[,i] =  exp_mod_Gaussian_resp_inv(x = seq(from = 1, to = 5000, by = 1),
                                            c = vmat[i, "c"],
    ...
    dist_quantiles <- cbind(as.vector(seq(from = 1, to = 5000, by = 1)),
  ...
  sims_physio <- simulate_mle(fittedModel = physio_model_eval[[length(data_list)]][[1]],
                              ...
                              sim_seed = 123,
                              fittedModel_curve_name = "exp_mod_Gauss_inv")
  ...
  legend("bottomright",
         legend = c(paste0("Simulated","\n",names(data_list)[length(data_list)]),
                    paste0(names(data_list)[length(data_list)])), pch = c(1,1), pt.cex = c(1,1),
         bty = "n",
         col = c("black","black"),
         lty = c(0,1),
         pt.lwd = c(1, 1),
...
#  detach("package:RMKdiscrete", character.only = TRUE)
  ...
  return(list(mod_physio_expmG_inv = physio_model_eval[[length(data_list)]][[1]]))
}
