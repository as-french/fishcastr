---
title: "An example of seasonal forecast development for anadromous brown trout smolts: Part I of IV"
subtitle: 
output: rmarkdown::html_vignette
vignette: >
  %\VignetteEncoding{UTF-8}
  %\VignetteIndexEntry{An example of seasonal forecast development for anadromous brown trout smolts: Part I of IV}
...
#                   max(df_counts_order$ssmolt[df_counts_order$year == i],na.rm = TRUE))))
  ...
  fishcastr::step_style_polygon(xvals = df_counts_order$yday[df_counts_order$year == i & df_counts_order$mean_count > 0],
    ...
    fishcastr::step_style_polygon(xvals = df_counts_order$yday[df_counts_order$year == i & df_counts_order$ssmolt > 0],
                     y_vals_low = low_yvals,
                     y_vals_up = df_counts_order$ssmolt[df_counts_order$year == i & df_counts_order$ssmolt > 0],
...
legend("bottomright", legend = c("water temperature","2004 - 2019 mean"),col = c("red",col_alpha_black), pch = c(NA,15),lty = c(1,NULL),bty = "n",inset = c(0.1,-1.8), xpd = NA, cex = 1.5)
dev.off()
...
legend("bottomright", legend = c("Catchment discharge","1981 - 2019 mean"),col = c("blue",col_alpha_black), pch = c(NA,15),lty = c(1,NULL),bty = "n",inset = c(0.1,-4.5), xpd = NA, cex = 1.5)
dev.off()
...
plot(1:1000, data_photper[1:1000], type = "l", ylab = "photperiod (hours)", xlab = paste0("days after ",min(fishcastr::data_ssmolt[["date"]])))
dev.off()
...
plot(1:365, data_lunar[1:365], type = "l", ylab = "moonlight exposure",xlab = paste0("days after ",min(fishcastr::data_ssmolt[["date"]])))
dev.off()
