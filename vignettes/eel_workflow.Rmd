...
In Burrishoole, the silver eel run invariably peaks in Autumn, but seaward migrating silver eels have been recorded during all months of the year. Therefore, we had to define a cut-off date to separate counts among calendar years into distinct runs (or approximately distinct "silvering years"). We defined a variable cut-off date that always occurred during spring, but varied among years based on the theoretical end date of the preceding year's run. We assumed that the active migration of any eels that had silvered prior to that date but not yet successfully migrated would cease. We defined the cut-off date as the first date after the 1^st^ of January on which daily mean water temperature for the preceding 10 days had exceeded 11 \degree C. This temperature threshold was chosen because it approximately marked the point at which a reversal of silvering characteristics has been observed in wild silvered eels held in captivity at ambient temperature [@durif_seasonal_2009]. While our definition of eel silvering year allows the total number of days in each forecast year to vary throughout the dataset, we used the 124^th^ calendar day of the year (i.e., the earliest eel year start date at Burrishoole, or earliest "biofix"; 4^th^ May 2011) as a reference first day of each silvering year with which to compare migration timing among years. See **Table \@ref(tab:appendix-s1-table)** for eel silvering year details - noting these bracketing dates were used to stratify census data into discrete "pre-migrant to active migrant silvering periods". As explained previously, *forecast* years for all species began at winter solstice as this date was used to initialise accumulation of photoperiod-weighted degree days.
...
saveRDS(eel_bio_year_tab, file = paste0(dirName, "eel_year_table.rds"))

rm(list = ls(all.names = TRUE))
invisible(gc())
```

(ref:appendix-s1-table) *European eel (*A. anguilla*) silvering years defined for Burrishoole between 1981 and 2018. Silvering year start dates and end dates are defined concurrently. Silvering year start dates are denoted by the first day of the calendar year that follows a consecutive period of 10 days during which water temperatures exceeded 11 degrees Celsius.*
...
eel_year_table <- readRDS(paste0(system.file("extdata", package = "fishcastr"),
                                 "/vignette_tables/eel_year_table.rds"))

tab <- knitr::kable(eel_year_table,row.names = FALSE, format = "html",
                    caption = "(ref:appendix-s1-table)",
                    booktabs = TRUE)

kableExtra::kable_classic(kable_input = tab,
                                  full_width = FALSE,html_font = "Arial", font_size = 12)
```
...
tab <- knitr::kable(res_tab,row.names = FALSE,format = "html", caption = "(ref:figure-5b-eel-result-tab-eel-1)",
                    booktabs = TRUE)
