---
title: 'Appendix S3: A seasonal forecasting workflow for the seaward migrations of anadromous brown trout smolts'
subtitle: |
...
vignette: |
  ...
  %\VignetteIndexEntry{Appendix S3: A seasonal forecasting workflow for the seaward migrations of anadromous brown trout smolts}
...
This appendix is included alongside the manuscript to complement descriptions of data included the manuscript and provide model validation plots specifically related to a model chain for forecasting the timing of anadromous brown trout smolt (*Salmo trutta* (Linnaeus, 1758)) migration. See **Appendix S1** for aspects of the seasonal forecasting workflow, which are shared among species; namely:
...
Here, we present complete seaward migrating anadromous brown trout smolt counts recorded between 1981 and 2019 in Burrishoole. **Figure \@ref(fig:catchment-specific-data-1-trout)** illustrates the width of the temporal window (~100 consecutive calendar days) during which downstream migrating trout smolts are expected in the Burrishoole fish traps. Note the moderate dispersion of annual counts relative to the phenology curve described by the 1981 - 2019 average daily count.

```{r catchment-specific-data-fish, out.width="100%", eval=FALSE, echo = FALSE}
# ---------------------------------------------------------------------------------------------------------- #
# FISH COUNTS ----
# ---------------------------------------------------------------------------------------------------------- #
...
data_stsmolt$year <-lubridate::year(data_stsmolt$date)
data_stsmolt$yday <-lubridate::yday(data_stsmolt$date)
data_stsmolt <- data_stsmolt[data_stsmolt$year >= 1981,]

# calculate average per year day
mean_daily_count <- tapply(data_stsmolt$stsmolt,INDEX = data_stsmolt$yday,FUN = mean,na.rm = TRUE)
lower_pct <- tapply(data_stsmolt$stsmolt,INDEX = data_stsmolt$yday,FUN = quantile, probs = 0.025,na.rm = TRUE)
upr_pct <- tapply(data_stsmolt$stsmolt,INDEX = data_stsmolt$yday,FUN = quantile,probs = 0.975,na.rm = TRUE)
...
df_counts <- merge(data_stsmolt,df_mean, by = "yday",all.x = TRUE)
df_counts_order <- df_counts[order(df_counts$date),]

  # for plotting clarity replace zeros with NAs
  ####
      df_counts_order$stsmolt[1] <- ifelse(df_counts_order$stsmolt[1] == 0 & df_counts_order$stsmolt[2] == 0, NA, df_counts_order$stsmolt[1])

      for(j in 2:(length(df_counts_order$stsmolt)-1)){
        df_counts_order$stsmolt[j] <- ifelse(df_counts_order$stsmolt[j] == 0 & df_counts_order$stsmolt[j-1] == 0 & df_counts_order$stsmolt[j+1] == 0, NA, df_counts_order$stsmolt[j])
      }

      df_counts_order$stsmolt[length(df_counts_order$stsmolt)] <- ifelse(df_counts_order$stsmolt[length(df_counts_order$stsmolt)] == 0 & df_counts_order$stsmolt[length(df_counts_order$stsmolt)-1] == 0, NA, df_counts_order$stsmolt[length(df_counts_order$stsmolt)])
  ####
...
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
dir.create(dirName, showWarnings = TRUE, mode = "0777")

png(filename = paste0(dirName,"catchment_specific_data-1-trout.png"), height = 2300, width = 1500, res =300)
...
for(i in unique(df_counts_order$year)) {
  ylims <- c(0,max(c(max(df_counts_order$upr[df_counts_order$year == i],na.rm = TRUE),
                   max(df_counts_order$stsmolt[df_counts_order$year == i],na.rm = TRUE))))
        low_yvals <- ifelse(is.na(df_counts_order$stsmolt[df_counts_order$year == i]),NA, 0)
  low_yvals_ave <- ifelse(is.na(df_counts_order$lwr[df_counts_order$year == i]),NA, 0)
        
  ylims_upper <- ifelse(i %in% c(1981:1993),1500,
                 ifelse(i %in% c(1994:2006),600,
                 ifelse(i %in% c(2007:2019),150,
                 1500)))
  
 # ylims <- c(0,ylims_upper)
...
#    }
  ...
  lines(df_counts_order$yday[df_counts_order$year == i &
          ...
          df_counts_order$mean_count[df_counts_order$year == i &
                                             df_counts_order$mean_count > 0], lwd = 0.5, col = col_alpha_black,
        type = "s")
  ...
  fishcastr::step_style_polygon(
    ...
    y_vals_up = ifelse(is.na(low_yvals), NA,
                       df_counts_order$stsmolt[df_counts_order$year == i]),
...
mtext(side = 2,text = expression(italic("S. trutta ")~smolt~count),outer = TRUE,line = 0)
...
```{r catchment-specific-data-1-trout, out.width="70%", eval = TRUE, fig.align = 'center', fig.cap = "(ref:catchment-specific-data-1-trout)", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"catchment_specific_data-1-trout.png"))
```

(ref:catchment-specific-data-1-trout) *Migration phenology curves (relative to 1981 - 2019 average) described by the combined daily counts of anadromous brown trout (*Salmo trutta*) recorded leaving the two freshwater outflows from Lough Feeagh and entering a tidal lagoon at Burrishoole, Ireland.*

*Stratifying trout smolt census data into forecast years*

For trout smolts, we used the winter solstice to stratify forecast years. However, we acknowledged that anadromy of *Salmo trutta* is flexible [@archer_interplay_2019; @nevoux_environmental_2019; @ferguson_anadromy_2019] and life histories in Burrishoole range from potadromous trout to estuary migratory, and marine migratory trout [@poole_characteristics_2006]. Our dataset contained small numbers of late summer migrating trout smolts that were recorded according to their silvered appearance as sea trout smolts, and we assumed these fish were late migrants adopting the same strategy as spring silvered migrants, as opposed to the separate cohort of unsilvered pre-smolts and or potadromous fish, which migrate in autumn in Burrishoole to full marine conditions amongst the following spring's smolt cohort or remain in the river or tidal lagoon immediately downstream of the traps (see @aarestrup_another_2018 for a discussion of uncertainties surrounding juvenile trout autumn migration).

*Observed migration timing anomalies and phenology curves*

To clarify context regarding anomaly definitions, we are skipping ahead to one of the outputs of workflow step 3. In **Figure \@ref(fig:catchment-specific-data-1b-trout)**, we present observed (tercile based) anomalies for the mean date of migration of anadromous brown trout smolts in Burrishoole (NOTE - for comparison with later seasonal forecast anomalies, computations of migration summary statistics are based solely on fish counts during forecast year "target season"; i.e., February to August for trout smolts). These data are not detrended, although the fishcastr package facilitates *linear* detrending of terciles; i.e., adjusting the 1/3 and 2/3 percentiles positions according to a fitted linear regression. In the presence of multidecadal trends in migration timing, linear detrending can be useful for assessing a seasonal forecast system's propensity to discriminate between occurrence and non-occurrence of anomalies, when the definition of anomalies for each year are adjusted relative to expectations given "contemporary conditions". Detrending is also useful for exploring phenotypic plasticity [@iler_detrending_2017] by answering questions such as: *Is inter-annual variability in timing anomalies more or less variable during 2000-2010 that variability during 1980-2000?*; i.e., *Do Atlantic salmon smolts have a greater or lesser propensity to adjust their migration timing to match seasonal conditions?*. This question assumes inter-annual variability in seasonal climate is consistent throughout the time series, although quantification of inter-annual variability in seasonal climate for selected windows could be used to adjust for any differences.

```{r observed-anomalies-anadromous-brown-trout, eval = FALSE, echo = FALSE}
# ---------------------------------------------------------------------------------------------------------- #
...
# trout smolts are counted in all months, but primarily from February through August
data_stsmolt$month <- lubridate::month(data_stsmolt$date)
tapply(data_stsmolt$stsmolt, data_stsmolt$month, sum)

# merge data and subset to target season
data_counts <- merge(data_stsmolt,data_stsmolt_forecast_years, by = "date", all.x = TRUE)
data_counts$salmonid_year[data_counts$date %in% seq(from = as.Date("2019-02-01"), to = as.Date("2019-12-21"), by = "day")] <- 2019
data_counts <- data_counts[data_counts$date <= as.Date("2019-12-21"),]
data_counts$salmonid_yday[data_counts$date >= as.Date("2019-02-01")] <- seq(from = 42, by = 1, length.out = (length(data_counts$salmonid_year[data_counts$salmonid_year %in% 2019])-41))

data_counts_trout <- data_counts[data_counts$salmonid_year %in% c(1981:2019),]
data_counts_trout$month <- lubridate::month(data_counts_trout$date)
data_counts_trout_target_s <- data_counts_trout
data_counts_trout_target_s$stsmolt[data_counts_trout_target_s$month %in% c(1,9:12)] <- 0
...
dirName <- paste0(system.file("extdata",package = "fishcastr"),
                      "/trout_wflow_figs/", sep = "", collapse = NULL)
dir.create(dirName, showWarnings = TRUE, mode = "0777")

png(file = paste0(dirName,"catchment_specific_data-1b-trout.png"), width=4500, height=6500, res=150)
trout_anoms <- fishcastr::ridgeline_plot_phenology(bio_year = data_counts_trout_target_s$salmonid_year,
                                 counts = data_counts_trout_target_s$stsmolt,
                                 yday = data_counts_trout_target_s$salmonid_yday,
                                 dates = data_counts_trout_target_s$date,
                                 method = "mean",
                                 species_name = "Sea trout (smolt)",
                                 season = "spring",
                                 plot = TRUE,
                                 detrended = FALSE,
                                 xlims_plot = c(0,1))
...
image <- magick::image_read(paste0(dirName,"catchment_specific_data-1b-trout.png"))
image_trimmed <- magick::image_trim(image) # trim white space
magick::image_write(image_trimmed, paste0(dirName,"catchment_specific_data-1b-trout_trim.png"))
```

```{r catchment-specific-data-1b-trout, out.width="70%", eval = TRUE, fig.align = 'center', fig.cap = "(ref:catchment-specific-data-1b-trout)", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"catchment_specific_data-1b-trout_trim.png"))
```

(ref:catchment-specific-data-1b-trout) *Migration phenology curves and migration timing anomalies (of mean forecast year day) described by the combined daily counts of anadromous brown trout (*Salmo trutta*) recorded leaving the two freshwater outflows from Lough Feeagh and entering a tidal lagoon at Burrishoole, Ireland.*
...
png(filename = paste0(dirName,"catchment_specific_data-2-trout.png"), height = 1500, width = 2000, res =300)
...
```{r catchment-specific-data-2-trout, out.width="70%", eval=FALSE, fig.align = 'center', fig.cap = "\\textit{Lough Feeagh AWQMS recorded water temperature at 2m depth (relative to 2004 - 2019 average).}", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"catchment_specific_data-2-trout.png"))
```
...
# plot data
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
png(filename = paste0(dirName,"catchment_specific_data-3-trout.png"), height = 2300, width = 1500, res =300)
...
# dates <- data_stsmolt$date
# latitude <- 53.932458
# longitude <- -9.575556
```

```{r catchment-specific-data-3-trout, out.width="70%", eval=FALSE, fig.align = 'center', fig.cap = "\\textit{Rating curve estimated catchment discharge from the combined outflows of Lough Feeagh (relative to 1981 - 2019 average).}", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"catchment_specific_data-3-trout.png"))
```
...
data_photper <- fishcastr::photper_calc(dates = fishcastr::import_fish_data(species = "stsmolt")[["date"]][1:1000],
                                        latitude = 53.932458,
                                        longitude = -9.575556)
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
png(filename = paste0(dirName,"lunar_solar_data-1-trout.png"), height = 1000, width = 1500, res =300)
plot(fishcastr::import_fish_data(species = "stsmolt")[["date"]][1:1000], data_photper[1:1000], type = "l", ylab = "photperiod (hours)", xlab = paste0("days after ",min(fishcastr::import_fish_data(species = "stsmolt")[["date"]])))
dev.off()
#knitr::include_graphics(path = "trout_wflow_figs/lunar_solar_data-1-trout.png")

# ---------------------------------------------------------------------------------------------------------- #
# CATCHMENT MOONLIGHT EXPOSURE (see: R/lunar_light_exposure.R) ----
# subset of dates here as long to calculate
# data_lunar <- fishcastr::lunar_light_exposure(date_of_interest = fishcastr::import_fish_data(species = "stsmolt")[["date"]][1:365],
#                                               lat = 53.932458,
#                                               lon = -9.575556,
#                                               log_result = FALSE)
# 
# dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
# png(filename = paste0(dirName,"lunar_solar_data-2-trout.png"), height = 1000, width = 1500, res =300)
# plot(fishcastr::import_fish_data(species = "stsmolt")[["date"]][1:365], data_lunar[1:365], type = "l", ylab = "moonlight exposure",xlab = paste0("days after ",min(fishcastr::import_fish_data(species = "stsmolt")[["date"]])))
# dev.off()

# ---------------------------------------------------------------------------------------------------------- #
data_lunar <- fishcastr::lunar_light_exposure(date_of_interest = fishcastr::import_fish_data(species = "stsmolt")[["date"]],
                                              lat = 53.932458,
                                              lon = -9.575556,
                                              log_result = FALSE)
data_lunar_df <- data.frame("date" = fishcastr::import_fish_data(species = "stsmolt")[["date"]],
...
png(filename = paste0(dirName,"lunar_solar_data-3-trout.png"), height = 2300, width = 1500, res =300)
par(mfcol = c(13,3),oma = c(8,4,1,1), mar = c(0.5,0.5,0.1,0.1))
for(i in 1981:2019) {
  ylims <- c(0,max(c(max(df_temps_order$upr[df_temps_order$year == i],na.rm = TRUE),
...
```{r lunar-solar-data-2-trout, out.width="70%", eval=FALSE, fig.align = 'center', fig.cap = "\\textit{(Clear-sky) Moonlight exposure estimated for the Burrishoole catchment calculated using suncalc::getSunlightTimes, suncalc::getMoonIllumination and suncalc::getMoonPosition.}", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"lunar_solar_data-3-trout.png"))
```
...
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
png(filename = paste0(dirName,"access_climate_data-1-trout_",x,".png"), height = 2300, width = 1500, res =300)
par(mfcol = c(13,3),oma = c(8,4,1,1), mar = c(0.5,0.5,0.1,0.1))
for(i in 1981:2019) {
...
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
png(filename = paste0(dirName,"access_climate_data-2-trout_",x,"_reanalysis.png"), height = 2300, width = 1500, res =300)
par(mfcol = c(13,3),oma = c(8,4,1,1), mar = c(0.5,0.5,0.1,0.1))
for(i in 1981:2019) {
  ylims <- c(0,max(c(max(df_met_order$upr[df_met_order$year == i],na.rm = TRUE),
                   max(df_met_order$tas[df_met_order$year == i], na.rm = TRUE))))
...
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
png(filename = paste0(dirName,"access_climate_data-3-trout_",x,"_SF.png"), height = 1700, width = 1700, res =300)
...
```{r access-climate-data-1-trout_tas, out.width="70%", eval=FALSE, fig.align = 'center', fig.cap = "\\textit{Recorded mean daily air temperature at the manual meteorological station in Burrishoole (relative to 1981 - 2019 average).}",echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"access_climate_data-1-trout_tas_manual.png"))
```

```{r access-climate-data-1-trout_pr, out.width="70%", eval=FALSE, fig.align = 'center', fig.cap = "\\textit{Recorded daily total precipitation at the manual meteorological station in Burrishoole (relative to 1981 - 2019 average).}",echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"access_climate_data-1-trout_pr_manual.png"))
```

```{r access-climate-data-1-trout_petH, out.width="70%", eval=FALSE, fig.align = 'center', fig.cap = "\\textit{Estimated daily potential evapotranspiration in Burrishoole estimated using the Hargreaves-Samani equation (relative to 1981 - 2019 average).}",echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"access_climate_data-1-trout_petH_manual.png"))
```

### *Climate reanalysis*

For each retrospective seasonal trout smolt migration forecast, we append two years (antecedent) bias adjusted ERA5 climate reanalysis up to the end of January to SEAS5 seasonal climate forecasts from February to August - hence the cessation of ERA5 data in early-2019 illustrated in **Figure \@ref(fig:access-climate-data-2-trout-tas)**. The data presented here are bias adjusted relative to local meteorological observations; therefore, the long run average climate illustrated by shading of **Figure \@ref(fig:access-climate-data-2-trout-tas)** are consistent with meteorological observation long run averages.

```{r access-climate-data-2-trout-tas, out.width="70%", eval = TRUE, fig.align = 'center', fig.cap = "(ref:access-climate-data-2-trout-tas)",echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"access_climate_data-2-trout_tas_reanalysis.png"))
```

(ref:access-climate-data-2-trout-tas) *ECMWF ERA5 modelled mean daily air temperature at 2m (calculated from hourly resolution ERA5) in Burrishoole (relative to 1981 - 2019 average).*

```{r access-climate-data-2-trout-pr, out.width="70%", eval=FALSE, fig.align = 'center', fig.cap = "\\textit{ECMWF ERA5 modelled daily total precipitation in Burrishoole (relative to 1981 - 2019 average).}",echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"access_climate_data-2-trout_pr_reanalysis.png"))
```

```{r access-climate-data-2-trout-petH, out.width="70%", eval=FALSE, fig.align = 'center', fig.cap = "\\textit{Hargreaves-Samani equation estimated daily potential evapotranspiration calculated from ECMWF ERA5 modelled daily minimum and maximum temperatures (extracted from hourly resolution ERA5) in Burrishoole (relative to 1981 - 2019 average).}",echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"access_climate_data-2-trout_petH_reanalysis.png"))
```

### *Seasonal climate forecast*

We accessed SEAS5 seasonal climate forecasts initialised on February 1 from which we retained data up to August 31. **Figure \@ref(fig:access-climate-data-3-trout-tas)** illustrates ensemble spread relative to long run average (1993 - 2019) ensemble spread; that is, following bias adjustment, long run average ensemble spread (illustrated by shading in **Figure \@ref(fig:access-climate-data-3-trout-tas)**) is consistent with long run average climate calculated from local meteorological observations.

```{r access-climate-data-3-trout-tas, out.width="70%", eval = TRUE, fig.align = 'center',fig.cap = "(ref:access-climate-data-3-trout-tas)", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"access_climate_data-3-trout_tas_SF.png"))
```

(ref:access-climate-data-3-trout-tas) *ECMWF SEAS5 modelled mean daily air temperature at 2m (calculated from 6-hourly resolution SEAS5) in Burrishoole (relative to 1993 - 2019 average).*

```{r access-climate-data-3-trout-pr, out.width="70%", eval=FALSE, fig.align = 'center',fig.cap = "\\textit{ECMWF SEAS5 modelled daily total precipitation in Burrishoole (relative to 1993 - 2019 average).}", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"access_climate_data-3-trout_pr_SF.png"))
```

```{r access-climate-data-3-trout-petH, out.width="70%", eval=FALSE, fig.align = 'center',fig.cap = "\\textit{Hargreaves-Samani equation estimated daily potential evapotranspiration calculated from ECMWF SEAS modelled daily minimum and maximum temperatures (extracted from 6-hourly resolution SEAS5) in Burrishoole (relative to 1993 - 2019 average).}", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"access_climate_data-3-trout_petH_SF.png"))
```
...
Descriptions of model functional form and fitting procedure may be found in **Appendix S1: Section 2b**. Below, **Figure \@ref(fig:model-proxy-preparedness-1-trout)** illustrates the extent of consistency between fitted "migration preparedness" model simulations and observations of standardised (scaled) daily counts vs. photoperiod-weighted degree days. In **Figure \@ref(fig:model-proxy-preparedness-2-trout)**, we present model checking plots.

```{r model-proxy-preparedness, eval=FALSE, echo = FALSE}
# ---------------------------------------------------------------------------------------------------------- #
# CALIBRATE MIGRATION PREPAREDNESS MODEL (see: data_raw/model_physio_expmG_trout_calibration.R) ----
model_physio_expmG_trout <- fishcastr::model_physio_expmG_trout
```

```{r model-proxy-preparedness-1-trout, out.width="60%", eval = TRUE, fig.align = 'center', fig.cap = "(ref:model-proxy-preparedness-1-trout)",echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                  "/vignette_figures/migration_prep_models/trout/")
knitr::include_graphics(path = paste0(dirName,"genpois_trout.png"))
```

(ref:model-proxy-preparedness-1-trout) *Fitted exponentially modified Gaussian relationship between photoperiod-weighted degree days and scaled daily counts of anadromous brown trout salmon (*Salmo trutta*) smolts recorded in the freshwater outflow traps of the Burrishoole catchment. Observations are colour-faceted by decade and are illustrated relative to the positive y axis. Simulations from the model fitted to data from 1981 - 1992 are mirrored on the negative y axis.*

```{r model-proxy-preparedness-2-trout, fig.show = "hold", out.width="40%", eval = TRUE, fig.align = 'center', fig.cap = "(ref:model-proxy-preparedness-2-trout)",echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
...
(ref:model-proxy-preparedness-2-trout) *Model checking plots for fitted exponentially modified Gaussian relationship between photoperiod-weighted degree days and scaled daily counts of anadromous brown trout salmon (*Salmo trutta*) smolts recorded in the freshwater outflow traps of the Burrishoole catchment.*

## (2c) Model fish counts

### *Calculate predictors*

Prior to fitting glmmTMB models, we pre-processed the raw and model derived catchment environmental data included in `fishcastr`. Pre-processing steps included stratification of trout forecast years, log transformation of positive skewed predictors (moonlight exposure, discharge and migration preparedness). We included both the log transformed and non-transformed data of these three "likely candidates" in the initial modelling dataset, as it was not clear form the outset whether transformations would improve statistical fit and or computation of the fitting procedure. The inclusion of log transformed vs. non-transformed variables varied among species.
...
Residual plots for models trained using data for 1981 - 1992 were used to refine model structure for trout smolts (**Figure \@ref(fig:model-trout-counts-1981-1992-fit-trout-1)**). Residual plots for models trained using data for 1981 - 2018 were used to examine the extent to which the 1981- 1992 model structure remained appropriate for contemporary predictions (**Figure \@ref(fig:model-trout-counts-2019-fit-operational-trout-1)**).

```{r model-trout-counts-1981-1992-fit-model,eval = FALSE, echo = FALSE}
# ---------------------------------------------------------------------------------------------------------- #
# LOAD COUNT AND PREDICTOR DATA ----
# ---------------------------------------------------------------------------------------------------------- #
data_stsmolt_enviro_1981_2019 <- readRDS(paste0(system.file("extdata",
                                                    package = "fishcastr"),
...
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/vignette_figures/count_model_checks/trout/")
dir.create(dirName, showWarnings = TRUE, mode = "0777")
...
```{r model-trout-counts-1981-1992-fit-trout-1, fig.show = "hold", out.width="30%", eval = TRUE, fig.align = 'center', fig.cap = "(ref:model-trout-counts-1981-1992-fit-trout-1)", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                  "/vignette_figures/count_model_checks/trout/")
...
(ref:model-trout-counts-1981-1992-fit-trout-1) *Model validation plots for daily fish count model trained on data from 1981 - 1992.*

<!-- ### Fit refined model to data using leave-one-year-out approach -->
   
```{r model-trout-counts-1981-2018-fit-loo,eval = FALSE, echo = FALSE}            
# ---------------------------------------------------------------------------------------------------------- #
# LOAD COUNT AND PREDICTOR DATA ----
# ---------------------------------------------------------------------------------------------------------- #
data_stsmolt_enviro_1981_2019 <- readRDS(paste0(system.file("extdata",
...
```{r model-trout-counts-2019-fit-operational,eval = FALSE, echo = FALSE}
# ---------------------------------------------------------------------------------------------------------- #
# LOAD COUNT AND PREDICTOR DATA ----
# ---------------------------------------------------------------------------------------------------------- #
data_stsmolt_enviro_1981_2019 <- readRDS(paste0(system.file("extdata",
...
# Create vignette sub directories ------------------------------------------------------
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/vignette_figures/count_model_checks/trout/op_model/")
...
```{r model-trout-counts-2019-fit-operational-trout-1,fig.show = "hold", out.width="30%", eval = TRUE, fig.align = 'center',fig.cap = "(ref:model-trout-counts-2019-fit-operational-trout-1)", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                  "/vignette_figures/count_model_checks/trout/op_model/")
knitr::include_graphics(path = paste0(dirName,"FigS5a.png"))
knitr::include_graphics(path = paste0(dirName,"FigS5b.png"))
knitr::include_graphics(path = paste0(dirName,"FigS5c.png"))
knitr::include_graphics(path = paste0(dirName,"FigS5d.png"))
...
(ref:model-trout-counts-2019-fit-operational-trout-1) *Model validation plots for daily fish count model trained on data from 1981 - 2018.*

# (3) EVALUATE FORECAST PERFORMANCE

## Evaluating seasonal climate forecast skill

Prior to evaluating seasonal forecast skill for the trout model chain under ERA5 and SEAS5 forcing, we can evaluate forecast skill for specific SEAS5 meteorological forcing variables themselves. Consistent with convention, we specify a lead time (e.g., one month; February) and target season period (e.g., March to May) (**Figures \@ref(fig:seas-forecast-skill-SEAS5-spring-trout-tas)**, **\@ref(fig:seas-forecast-skill-SEAS5-spring-trout-pr)** and **\@ref(fig:seas-forecast-skill-SEAS5-spring-trout-petH)**).

```{r seas-forecast-skill-SEAS5-spring-trout, eval = FALSE, echo = FALSE}
# LOAD ERA 5 RDATA
ERA5_data_op_Jan_2019 <- fishcastr::grid_ERA5_1979_2019_Jan_bc

variables = c("tas","pr","petH")
ERA5_data_op_Feb_Aug_2019 <- fishcastr::grid_ERA5_1981_2019_2_3_4_5_6_7_8_tas_pr_petH_bc
ERA5_data_op_Feb_Aug_2019_m1 <- lapply(variables, function(x){
  res <- transformeR::subsetGrid(grid = ERA5_data_op_Feb_Aug_2019[[x]],
                          members = 1,
                          years = 2019)
  res_2 <- res[-c(5,6)] # remove member and initialisation dates information
  return(res_2)
  })
names(ERA5_data_op_Feb_Aug_2019_m1) <- variables

variables = c("tas","pr","petH")
ERA5_data_op_Sep_Dec_2019 <- fishcastr::grid_SEAS5_1993_2019_7_8_9_10_11_12_tas_pr_petH_bc
ERA5_data_op_Sep_Dec_2019_m1 <- lapply(variables, function(x){
  res <- transformeR::subsetGrid(grid = ERA5_data_op_Sep_Dec_2019[[x]],
                          members = 1,
                          years = 2019,
                          season = 8:12)
  res_2 <- res[-c(5,6)] # remove member and initialisation dates information
...
names(ERA5_data_op_Sep_Dec_2019_m1) <- variables

# bind grids by time
ERA5_data_op <- lapply(variables, function(x){
  transformeR::bindGrid(list(ERA5_data_op_Jan_2019[[x]],
                             ERA5_data_op_Feb_Aug_2019_m1[[x]],
                             ERA5_data_op_Sep_Dec_2019_m1[[x]]),
                        dimension = "time")
})
...
# load SEAS5 data
forecast.data <- fishcastr::grid_SEAS5_1993_2019_2_3_4_5_6_7_8_tas_pr_petH_bc
...
hind <- lapply(forecast.data, function(x)
  transformeR::subsetGrid(x, season = 3:5))
...
cofinogramdat <-
  lapply(variables, function(i) {
    filename = paste0(system.file("extdata", package = "fishcastr"),"/trout_wflow_figs/SEAS5_",paste0(target_season, collapse = "_"),"_",paste0(i, collapse = "_"),".png")
...
saveRDS(object = cofinogramdat,
        file = paste0(system.file("extdata", package = "fishcastr"),
                                              "/trout_wflow_figs/climate_terciles_",
...
image <- magick::image_read(paste0(system.file("extdata",package = "fishcastr"),
                      "/trout_wflow_figs/SEAS5_3_4_5_tas.png"))
image_trimmed <- magick::image_trim(image) # trim white space
magick::image_write(image_trimmed, paste0(system.file("extdata",package = "fishcastr"),
                      "/trout_wflow_figs/SEAS5_3_4_5_tas.png"))

image <- magick::image_read(paste0(system.file("extdata",package = "fishcastr"),
                      "/trout_wflow_figs/SEAS5_3_4_5_pr.png"))
image_trimmed <- magick::image_trim(image) # trim white space
magick::image_write(image_trimmed, paste0(system.file("extdata",package = "fishcastr"),
                      "/trout_wflow_figs/SEAS5_3_4_5_pr.png"))

image <- magick::image_read(paste0(system.file("extdata",package = "fishcastr"),
                      "/trout_wflow_figs/SEAS5_3_4_5_petH.png"))
image_trimmed <- magick::image_trim(image) # trim white space
magick::image_write(image_trimmed, paste0(system.file("extdata",package = "fishcastr"),
                      "/trout_wflow_figs/SEAS5_3_4_5_petH.png"))
...
```{r seas-forecast-skill-SEAS5-spring-trout-tas, out.width="70%", eval = TRUE, fig.align = 'center',fig.cap = "(ref:seas-forecast-skill-SEAS5-spring-trout-tas)", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                  "/trout_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"SEAS5_3_4_5_tas.png"))
```

(ref:seas-forecast-skill-SEAS5-spring-trout-tas) *Predictive performance of SEAS5 seasonal climate forecast for seasonal mean daily mean air temperature during the target season (March to May). Forecasts have a lead time of one month (i.e., initialisation on 1 February).*

```{r seas-forecast-skill-SEAS5-spring-trout-pr, out.width="70%", eval = TRUE, fig.align = 'center',fig.cap = "(ref:seas-forecast-skill-SEAS5-spring-trout-pr)", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                  "/trout_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"SEAS5_3_4_5_pr.png"))
```

(ref:seas-forecast-skill-SEAS5-spring-trout-pr) *Predictive performance of SEAS5 seasonal climate forecast for seasonal total precipitation during the target season (March to May). Forecasts have a lead time of one month (i.e., initialisation on 1 February).*

```{r seas-forecast-skill-SEAS5-spring-trout-petH, out.width="70%", eval = TRUE, fig.align = 'center', fig.cap = "(ref:seas-forecast-skill-SEAS5-spring-trout-petH)",echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                  "/trout_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"SEAS5_3_4_5_petH.png"))
```

(ref:seas-forecast-skill-SEAS5-spring-trout-petH) *Predictive performance of SEAS5 seasonal climate forecast for seasonal total potential evapotranspiration during the target season (March to May). Forecasts have a lead time of one month (i.e., initialisation on 1 February).*

## (3a) Generate migration forecast ensembles

### *Known conditions (ECMWF's ERA5)*

We use `fishcastr` to produce a 25 member ensemble of plausible anadromous brown trout smolt daily counts under known conditions. We consider the model chain forcing data a "synthetic ensemble" of bias corrected ECMWF ERA5 data, whereby each member for a given forecast year is identical, but each year contains respective reanalysis data for that year. Owing to the simulation based approach, ensemble fish counts are produced stochastically and are effectively random draws from the (generalised Poisson) conditional distribution; thus each member of the output fish count ensemble is unique.

```{r figure-5a-trout-methods, warning = FALSE, eval = FALSE, echo = FALSE}

# load physio response model
mod_physio_expmG <- fishcastr::model_physio_expmG_trout
...
model_list_1of2 <- readRDS(paste0(system.file("extdata", package = "fishcastr"),
                                   "/vignette_data/mTrain_genpois_trout_list_1of2.rds"))
model_list_2of2 <- readRDS(paste0(system.file("extdata", package = "fishcastr"),
                                   "/vignette_data/mTrain_genpois_trout_list_2of2.rds"))
...
mTrain_genpois_list[(1 + length(model_list_1of2)):(length(model_list_1of2)+length(model_list_2of2))] <- model_list_2of2
 ...
 # add 2019 data
 mTrain_genpois_trout_2019 <- model_list_1of2 <- readRDS(paste0(system.file("extdata", package = "fishcastr"),
                                    "/vignette_data/mTrain_genpois_trout_2019.rds"))
 pP_params_trout_2019 <- model_list_1of2 <- readRDS(paste0(system.file("extdata", package = "fishcastr"),
                                    "/vignette_data/pP_params_trout_2019.rds"))
 
 mTrain_genpois_list[[39]] <- list(mTrain_genpois_trout_2019,
                                 pP_params_trout_2019)
...
#   # save
# dir.data <-  paste0(getwd(),'/vignettes/vignette_data/ERA5_bcc_ideal_trout')
...
data_stsmolt_enviro_1981_2019 <- readRDS(paste0(system.file("extdata", package = "fishcastr"),
                                   "/vignette_data/data_stsmolt_enviro_1981_2019.rds"))
...
system.time({seasonal_forecast_list_dfs_stsmolt_daily_stoc <-
  fishcastr::compile_seasonal_forecast_list_glmm(
    ...
    counts_and_antecedent_conditions = data_stsmolt_enviro_1981_2019,
...
#174.14/60 # 3 mins for 3 years
 #1893.11/60 # 32 mins for 39 years

saveRDS(seasonal_forecast_list_dfs_stsmolt_daily_stoc,
        paste0(system.file("extdata",
                           package = "fishcastr"),
               "/vignette_data/SF_trout_ideal_loo_cv_23456.rds"))

#saveRDS(seasonal_forecast_list_dfs_stsmolt_daily_stoc,
#        paste0(getwd(),"/vignettes",
#               "/vignette_data/SF_trout_ideal_loo_cv.rds"))
...
seasonal_forecast_list_dfs_stsmolt_daily_stoc_era5[[39]] <-
  lapply(seasonal_forecast_list_dfs_stsmolt_daily_stoc_era5[[39]], function(x) {
    NA_dates_2019 <- x$date[which(is.na(x$stsmolt))]
    x$stsmolt[x$date %in% NA_dates_2019] <-
      data_stsmolt$stsmolt[data_stsmolt$date %in% NA_dates_2019]
    return(x)
  })
...
fish_counts_ERA5 <- fishcastr::plot_ensemble_data(seasonal_ensemble = seasonal_forecast_list_dfs_stsmolt_daily_stoc_era5,
                           pseudo_obs_ensemble = seasonal_forecast_list_dfs_stsmolt_daily_stoc_era5,
                           var_interest = "forecast_sim_counts",
                           obs_count_name = "stsmolt",
...
hydro_ERA5 <- lapply(seasonal_forecast_list_dfs_stsmolt_daily_stoc_era5, function(x){
...
wtemp_ERA5 <- lapply(seasonal_forecast_list_dfs_stsmolt_daily_stoc_era5, function(x){
 res <- lapply(x, function(y){
...
```{r era5-model-chain-checks-trout-plot-counts, fig.align = "center", out.width="70%",eval = TRUE, fig.cap = "(ref:era5-model-chain-checks-trout-plot-counts)", echo = FALSE}
knitr::include_graphics(paste0(system.file("extdata", package = "fishcastr"),
                               "/trout_wflow_figs/ERA5_forecast_sim_counts.png"))
```

(ref:era5-model-chain-checks-trout-plot-counts) *Daily resolution anadromous brown trout (*S. trutta*) smolt simulated counts recorded at Burrishoole during 1981 to 2019 during the seasonal forecast target season (February to July). Grey filled bars illustrate observations, while the 95% prediction intervals of simulated counts generated using known (held-out) model chain forcing data (ECMWF's ERA5 climate reanalysis) are illustrated by green polygons. The green line running through the polygon is the daily median simulated count. Daily median and 95% prediction intervals were calculated from 25 simulations per year. Note the different y-axis scales among plots.*

Further exploration of model chain predictions for hydrology (**Figure \@ref(fig:era5-model-chain-checks-trout-plot-discharge)**) and water temperature (**Figure \@ref(fig:era5-model-chain-checks-trout-plot-water-temp)**) are useful for identifying any systematic biases in seasonal prediction that are less clear when validating the model as a whole (e.g., observation vs. prediction plots and using KGE and rmse statistics).
```{r era5-model-chain-checks-trout-plot-discharge, fig.align = "center", out.width="70%",eval = TRUE,fig.cap = "(ref:era5-model-chain-checks-trout-plot-discharge)", echo = FALSE}
knitr::include_graphics(paste0(system.file("extdata", package = "fishcastr"),
                               "/trout_wflow_figs/ERA5_discharge.png"))
```

(ref:era5-model-chain-checks-trout-plot-discharge) *Daily resolution GR4J hydrologic model predictions and concurrent rating curve derived catchment discharge recorded at Burrishoole during 1981 to 2019 during the seasonal forecast antecedent, lead-time and target season for anadromous brown trout (*S. trutta*) smolt (December 20/21/22 to July 31). Black lines illustrate rating curve derived discharge, while blue lines illustrate GR4J predictions generated using known (held-out) model chain forcing data (ECMWF's ERA5 climate reanalysis). Note the different y-axis scales among plots.*

```{r era5-model-chain-checks-trout-plot-water-temp, fig.align = "center", out.width="70%",eval = TRUE,fig.cap = "(ref:era5-model-chain-checks-trout-plot-water-temp)", echo = FALSE}
knitr::include_graphics(paste0(system.file("extdata", package = "fishcastr"),
                               "/trout_wflow_figs/ERA5_Tw_mod.png"))
```

(ref:era5-model-chain-checks-trout-plot-water-temp) *Daily resolution statistical air to water temperature model predictions and concurrent measurements recorded at Burrishoole during 1981 to 2019 during the seasonal forecast antecedent, lead-time and target season for anadromous brown trout (*S. trutta*) smolt (December 20/21/22 to July 31). Black lines illustrate observations (1981 to 2003 paper chart recorder in Mill Race; 2004 - 2019 Lough Feeagh AWQMS 2m depth temperature), while red lines illustrate model predictions generated using known (held-out) model chain forcing data (ECMWF's ERA5 climate reanalysis). Note the different y-axis scales among plots.*
...
```{r figure-5b-trout-methods, warning = FALSE, fig.height=7, fig.width=7, eval = FALSE, echo = FALSE}
...
data_stsmolt_enviro_1993_2019 <- data_stsmolt_enviro_1981_2019[data_stsmolt_enviro_1981_2019$salmonid_year %in% c(1993:2019),]

#View(data_stsmolt_enviro_1981_2019)
...
system.time({seasonal_forecast_list_dfs_stsmolt_daily_stoc <-
  fishcastr::compile_seasonal_forecast_list_glmm(
    ...
    seasonal_forecast_grid = seas_subset,
    reanalysis_data = data_ERA5_1979_2019_Jan_bc,
    counts_and_antecedent_conditions = data_stsmolt_enviro_1993_2019,
    species_bio_year_name = "salmonid_year",
    species_bio_yday_name = "salmonid_yday",
    fish_reaction_time = 20,
    initialisation_month = 2,
...
#174.14/60 # 3 mins for 3 years
# 1312.89/60 # 22 mins for 27 years

saveRDS(seasonal_forecast_list_dfs_stsmolt_daily_stoc,
        paste0(system.file("extdata",
                           package = "fishcastr"),
               "/vignette_data/SF_trout_loo_cv_23456.rds"))

#saveRDS(seasonal_forecast_list_dfs_stsmolt_daily_stoc,
#        paste0(getwd(),"/vignettes",
#               "/vignette_data/SF_trout_loo_cv.rds"))
...
The following **Figures \@ref(fig:SEAS5-model-chain-checks-trout-plot-counts)**, **\@ref(fig:SEAS5-model-chain-checks-trout-plot-discharge)** and **\@ref(fig:SEAS5-model-chain-checks-trout-plot-water-temp)** illustrate ensemble spread for simulated variable outputs from model chain components.
```{r SEAS5-model-chain-checks-trout, warning = FALSE, eval = FALSE, echo = FALSE}
# --------------------------------------------------------------------------------------- #
# LOAD SEASONAL FORECAST DATA (PREDICTIONS/SIMULATIONS/PESUEDO-OBSERVATIONS)
# --------------------------------------------------------------------------------------- #
seasonal_forecast_list_dfs_stsmolt_daily_stoc_SEAS5 <-
  readRDS(file = paste0(
...
seasonal_forecast_list_dfs_stsmolt_daily_stoc_SEAS5[[27]] <-
  lapply(seasonal_forecast_list_dfs_stsmolt_daily_stoc_SEAS5[[27]], function(x) {
    NA_dates_2019 <- x$date[which(is.na(x$stsmolt))]
    x$stsmolt[x$date %in% NA_dates_2019] <-
      data_stsmolt$stsmolt[data_stsmolt$date %in% NA_dates_2019]
    return(x)
...
fish_counts_SEAS5 <- plot_ensemble_data(seasonal_ensemble = seasonal_forecast_list_dfs_stsmolt_daily_stoc_SEAS5,
                           pseudo_obs_ensemble = seasonal_forecast_list_dfs_stsmolt_daily_stoc_SEAS5,
                           var_interest = "forecast_sim_counts",
                           obs_count_name = "stsmolt",
                           bio_yday = "salmonid_yday",
                           bio_year = "salmonid_year",
...
hydro_SEAS5 <- lapply(seasonal_forecast_list_dfs_stsmolt_daily_stoc_SEAS5, function(x){
 res <- lapply(x, function(y){
    z <- merge(y,calculated_Q,by = "date",all.x = TRUE)
    return(z)
  })
...
wtemp_SEAS5 <- lapply(seasonal_forecast_list_dfs_stsmolt_daily_stoc_SEAS5, function(x){
 res <- lapply(x, function(y){
    z <- merge(y,water_temp.merge.subset,by = "date",all.x = TRUE)
    return(z)
  })
...
```{r SEAS5-model-chain-checks-trout-plot-counts, fig.align = "center", out.width="70%",eval = TRUE, fig.cap = "(ref:SEAS5-model-chain-checks-trout-plot-counts)",echo = FALSE}
knitr::include_graphics(paste0(system.file("extdata", package = "fishcastr"),
                               "/trout_wflow_figs/SEAS5_forecast_sim_counts.png"))
```

(ref:SEAS5-model-chain-checks-trout-plot-counts) *Daily resolution anadromous brown trout (*S. trutta*) smolt simulated counts recorded at Burrishoole during 1993 to 2019 during the seasonal forecast target season (February to July). Grey filled bars illustrate observations, while the 95% prediction intervals of simulated counts generated using seasonal forecast model chain forcing data (ECMWF's SEAS5 seasonal forecast system) are illustrated by green polygons. The green line running through the polygon is the daily median simulated count. Daily median and 95% prediction intervals were calculated from 25 simulations per year. Note the different y-axis scales among plots.*

```{r SEAS5-model-chain-checks-trout-plot-discharge, fig.align = "center", out.width="70%",eval = TRUE, fig.cap = "(ref:SEAS5-model-chain-checks-trout-plot-discharge)",echo = FALSE}
knitr::include_graphics(paste0(system.file("extdata", package = "fishcastr"),
                               "/salmon_wflow_figs/SEAS5_discharge.png"))
```

(ref:SEAS5-model-chain-checks-trout-plot-discharge) *Daily resolution GR4J hydrologic model predictions and concurrent rating curve derived catchment discharge recorded at Burrishoole during 1993 to 2019 during the seasonal forecast antecedent, lead-time and target season for anadromous brown trout (*S. trutta*) smolt (December 20/21/22 to July 31). Black lines illustrate rating curve derived discharge, while blue polygons illustrate 95% prediction intervals of GR4J predictions generated using seasonal forecast model chain forcing data (ECMWF's SEAS5 seasonal forecast system). Note the different y-axis scales among plots.*

```{r SEAS5-model-chain-checks-trout-plot-water-temp, fig.align = "center", out.width="70%",eval = TRUE, fig.cap = "(ref:SEAS5-model-chain-checks-trout-plot-water-temp)",echo = FALSE}
knitr::include_graphics(paste0(system.file("extdata", package = "fishcastr"),
                               "/salmon_wflow_figs/SEAS5_Tw_mod.png"))
```

(ref:SEAS5-model-chain-checks-trout-plot-water-temp) *Daily resolution statistical air to water temperature model predictions and concurrent measurements recorded at Burrishoole during 1981 to 2019 during the seasonal forecast antecedent, lead-time and target season for anadromous brown trout (*S. trutta*) smolt (December 20/21/22 to July 31). Black lines illustrate observations (1993 to 2003 paper chart recorder in Mill Race; 2004 - 2019 Lough Feeagh AWQMS 2m depth temperature), while red polygons illustrate 95% prediction intervals of model predictions generated using seasonal forecast model chain forcing data (ECMWF's SEAS5 seasonal forecast system). Note the different y-axis scales among plots.*

## (3b) Evaluate retrospective skill

Tabulated skill scores compactly reveal significant skill scores under ERA5 (**Table \@ref(tab:figure-5a-trout-result-tab-trout-1)**) and SEAS5 (**Table \@ref(tab:figure-5b-trout-result-tab-trout-1)**) forcing for a range of migration timing seasonal summary statistics. Percentile specific tercile plots for seasonal migration predictions under ERA5 (**Figure \@ref(fig:figure-5a-trout-plots-trout-2)**) and SEAS5 (**Figure \@ref(fig:figure-5b-trout-plots-trout-2)**) forcing reveal year-to-year accuracy of probabilistic predictions.

<!-- ### Known conditions (ECMWF's ERA5) -->

```{r figure-5a-trout-plots, warning = FALSE, fig.height=7, fig.width=7, eval = FALSE, echo = FALSE}

# load predictions in list format
seasonal_forecast_list_dfs_stsmolt_daily_stoc <- readRDS(paste0(system.file("extdata", package = "fishcastr"),
                                   "/vignette_data/SF_trout_ideal_loo_cv_23456.rds"))

#seasonal_forecast_list_dfs_stsmolt_daily_stoc <- readRDS(file = paste0(getwd(),"/vignettes/vignette_data\\SF_trout_ideal_loo_cv.rds"))
...
# --------------------------------------------------------------------------------------------------#
seas_fore_df_stsmolt_daily_full_obs <- do.call(rbind,lapply(X = seasonal_forecast_list_dfs_stsmolt_daily_stoc,
                                                  FUN = function(x){x[[1]]}))

seas_fore_df_stsmolt_daily <- seas_fore_df_stsmolt_daily_full_obs[seas_fore_df_stsmolt_daily_full_obs$salmonid_year <= 2019,]

# add observed 2019 smolts to df
#fishcastr::download_fish_data()
data_stsmolt <- fishcastr::import_fish_data(species = "stsmolt")

NA_dates_2019 <- seas_fore_df_stsmolt_daily$date[which(is.na(seas_fore_df_stsmolt_daily$stsmolt))]
data_stsmolt$stsmolt[data_stsmolt$date %in% NA_dates_2019]
seas_fore_df_stsmolt_daily$stsmolt[seas_fore_df_stsmolt_daily$date %in% NA_dates_2019] <- data_stsmolt$stsmolt[data_stsmolt$date %in% NA_dates_2019]

seas_fore_df_stsmolt_daily_tercile_data <-
  fishcastr::boxplot_phenology(
    bio_year = seas_fore_df_stsmolt_daily$salmonid_year,
    counts = seas_fore_df_stsmolt_daily$stsmolt,
    yday = seas_fore_df_stsmolt_daily$salmonid_yday,
    dates = seas_fore_df_stsmolt_daily$date,
    method = stat,
    species_name = "Sea trout (smolt)",
    season = "Spring",
    summary_stat_calculation_months = c(2:7),
    plot = FALSE,
    detrended = FALSE
  )

obs <- seas_fore_df_stsmolt_daily_tercile_data
...
seas_list <- plyr::llply(.data = 1:25,.fun = function(y){
  #y = 1
  seas_fore_df_stsmolt_daily_full <- do.call(rbind,lapply(X = seasonal_forecast_list_dfs_stsmolt_daily_stoc,
                                                         FUN = function(x){x[[y]]}))
  seas_fore_df_stsmolt_daily <- seas_fore_df_stsmolt_daily_full[seas_fore_df_stsmolt_daily_full$salmonid_year <= 2019,]
 # any(is.na(seas_fore_df_stsmolt_daily[[pred_or_sim]]))
  
  # 3. use existing function to calculate quantile trends in observed data
  seas_fore_df_stsmolt_daily_tercile_data_new <-
    fishcastr::boxplot_phenology(
      bio_year = seas_fore_df_stsmolt_daily$salmonid_year,
      counts = seas_fore_df_stsmolt_daily[[pred_or_sim]],
      yday = seas_fore_df_stsmolt_daily$salmonid_yday,
      dates = seas_fore_df_stsmolt_daily$date,
      method = stat,
      species_name = "Sea trout (smolt)",
 ...
 reforecasts <- seas_fore_df_stsmolt_daily_tercile_data_new$summary_stats
 return(list(reforecasts_list = reforecasts,
             mean_terciles = seas_fore_df_stsmolt_daily_tercile_data_new$mean_terciles))
})
 #
...
# TERCILE PLOT ----
# ------------------------------------------------------------------------------------------------------------- #
# dirName <- paste0(getwd(),"/vignettes/vignette_figures", "/Fig5a", "/trout/", sep = "",
#                  collapse = NULL)
#dir.create(dirName, showWarnings = FALSE, mode = "0777")

dirName <- paste0(system.file("extdata", package = "fishcastr"),
                                   "/vignette_figures/Fig5a/trout/")
dir.create(dirName, showWarnings = FALSE, mode = "0777")
...
type_of_pred <- ifelse(pred_or_sim == "forecast_sim_counts","sim","pred")
  ...
  png(file = paste0(dirName,"Fig5a_ERA5_trout_",type_of_pred,"_",stat,".png"), width=2100, height=2200, res=300)
terciles_summary_stat_plot <-
  ...
  return(list("individual_members"= seas_list,
              ...
              "seas_fore_df_stsmolt_daily_full_obs" = seas_fore_df_stsmolt_daily_full_obs,
              "tercile_anoms_sf_ensemble" = tercile_anoms_sf_ensemble))
...
sum_stats_list_subset <- lapply(sum_stats_list,function(x){
  res <- x[-c(which(names(x) == "seas_fore_df_stsmolt_daily_full_obs"))]
  return(res)
})

saveRDS(object = sum_stats_list_subset,
        file = paste0(dirName,"Fig5a_ERA5_trout_tercile_data.rds"))
...
dirName <- paste0(dir.data, "/", sep = "", collapse = NULL)
#dir.create(dirName, showWarnings = TRUE, recursive = TRUE, mode = "0777")
write.table(x = windows_op_table, file = paste0(dirName,"windows_op_ERA5_trout.csv"), row.names = TRUE, sep = ",",col.names = FALSE)
  
# --------------------------------------------------------------------------------------------------#
# PLOT ALL SIMULATIONS (uncomment if required) ----
# --------------------------------------------------------------------------------------------------#
#    dirName <- paste0(getwd(),"/vignettes/vignette_figures", "/Fig5a", "/trout/year_mem_plot/", sep = "",
#                   collapse = NULL)
# dir.create(dirName, showWarnings = FALSE, mode = "0777")
# 
# type_of_pred <- ifelse(pred_or_sim == "forecast_sim_counts","sim","pred")
...
#         forecast_list_object = seasonal_forecast_list_dfs_stsmolt_daily_stoc,
...
png(file = paste0(dirName_plots,"FigS6b_ridge_mean.png"), width=9000, height=13000, res=300)
trout_anoms <- fishcastr::ridgeline_plot_phenology(bio_year = sum_stats_list$mean$seas_fore_df_stsmolt_daily_full_obs$salmonid_year,
                                         counts = sum_stats_list$mean$seas_fore_df_stsmolt_daily_full_obs[[pred_or_sim]],
                                         yday = sum_stats_list$mean$seas_fore_df_stsmolt_daily_full_obs$salmonid_yday,
                                         dates = sum_stats_list$mean$seas_fore_df_stsmolt_daily_full_obs$date,
                                         method = "mean",
                                         species_name = "Sea trout (smolt)",
                                         season = "spring",
                                         plot = TRUE,
...
```{r figure-5a-trout-result-tab-trout-1, out.width="70%", eval = TRUE, fig.align = 'center', echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                                   "/vignette_figures/Fig5a/trout/")
res_tab <- read.table(file = paste0(dirName,"windows_op_ERA5_trout.csv"), sep = ",",
                      stringsAsFactors = FALSE)
colnames(res_tab) <- res_tab[1,]
res_tab <- res_tab[-1,]

tab <- knitr::kable(res_tab,row.names = FALSE, format = "html",
                    caption = "(ref:figure-5a-trout-result-tab-trout-1)",
                    booktabs = TRUE)

kableExtra::kable_classic(kable_input = tab, full_width = FALSE,html_font = "Arial")
#tab
```

(ref:figure-5a-trout-result-tab-trout-1) *Receiver Operating Characteristic Skill Scores (ROCSS) for tercile based anomalies of migration timing summary statistics under ECMWF ERA5 forcing.*

```{r figure-5a-trout-plots-trout-1, out.width="70%", eval=FALSE, warning=FALSE, fig.align = 'center', echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                  "/vignette_figures/Fig5a/trout/")

# ---------------------------------------------------------------------------------------------- #
# 5th ----
image <- magick::image_read(paste0(dirName,"Fig5a_ERA5_trout_sim_5th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
...
magick::image_write(image_chopped2, paste0(dirName,"Fig5a_ERA5_trout_sim_5th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 25th ----
image <- magick::image_read(paste0(dirName,"Fig5a_ERA5_trout_sim_25th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5a_ERA5_trout_sim_25th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 33rd ----
image <- magick::image_read(paste0(dirName,"Fig5a_ERA5_trout_sim_33rd.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5a_ERA5_trout_sim_33rd_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 50th ----
image <- magick::image_read(paste0(dirName,"Fig5a_ERA5_trout_sim_50th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5a_ERA5_trout_sim_50th_crop.png"))
...
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5a_ERA5_trout_sim_50th_crop_axis.png"))

# ---------------------------------------------------------------------------------------------- #
# 66th ----
image <- magick::image_read(paste0(dirName,"Fig5a_ERA5_trout_sim_66th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5a_ERA5_trout_sim_66th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 75th ----
image <- magick::image_read(paste0(dirName,"Fig5a_ERA5_trout_sim_75th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5a_ERA5_trout_sim_75th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 95th ----
image <- magick::image_read(paste0(dirName,"Fig5a_ERA5_trout_sim_95th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5a_ERA5_trout_sim_95th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# mean ----
image <- magick::image_read(paste0(dirName,"Fig5a_ERA5_trout_sim_mean.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x400+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5a_ERA5_trout_sim_mean_crop.png"))
...
multipanelfigure::fill_panel(paste0(dirName,"Fig5a_ERA5_trout_sim_5th_crop.png"), row = 1,  column = 1,scaling = "shrink", panel_clip = "on",label = "5th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5a_ERA5_trout_sim_25th_crop.png"), row = 2,  column = 1,scaling = "shrink", panel_clip = "on",label = "25th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5a_ERA5_trout_sim_33rd_crop.png"), row = 3,  column = 1,scaling = "shrink", panel_clip = "on",label = "33rd") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5a_ERA5_trout_sim_50th_crop.png"), row = 4,  column = 1,scaling = "shrink", panel_clip = "on",label = "50th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5a_ERA5_trout_sim_66th_crop.png"), row = 5,  column = 1,scaling = "shrink", panel_clip = "on",label = "66th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5a_ERA5_trout_sim_75th_crop.png"), row = 6,  column = 1,scaling = "shrink", panel_clip = "on",label = "75th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5a_ERA5_trout_sim_95th_crop.png"), row = 7,  column = 1,scaling = "shrink", panel_clip = "on",label = "95th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5a_ERA5_trout_sim_mean_crop.png"), row = 8,  column = 1,scaling = "shrink", panel_clip = "on",label = "mean") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5a_legcrop.png"), row = 9,  column = 1,scaling = "shrink", panel_clip = "on",label = "")
Fig5a_ERA5
invisible(dev.off())
```

```{r figure-5a-trout-plots-trout-2, out.width="70%", eval = TRUE, warning=FALSE, fig.align = 'center',fig.cap = "(ref:figure-5a-trout-plots-trout-2)", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                  "/vignette_figures/Fig5a/trout/")
knitr::include_graphics(path = paste0(dirName,"Fig5a_ERA5.png"))
```

(ref:figure-5a-trout-plots-trout-2) *Tercile plots illustrating the model chain's propensity to correctly predict anomaly categories during the retrospective period (1981 - 2019) using ECMWF's ERA5 as forcing data.*

<!-- *### Seasonal forecast (ECMWF's SEAS5)* -->

```{r figure-5b-trout-plots, warning = FALSE, fig.height=7, fig.width=7, eval = FALSE, echo = FALSE}
library(fishcastr)
...
#dev.new()
seas_fore_df_stsmolt_daily_tercile_data <- fishcastr::boxplot_phenology(bio_year = seas_fore_df_stsmolt_daily$salmonid_year,
                                                            counts = seas_fore_df_stsmolt_daily$stsmolt,
                                                            yday = seas_fore_df_stsmolt_daily$salmonid_yday,
                                                            dates = seas_fore_df_stsmolt_daily$date,
                                                            method = stat,
...
# ------------------------------------------------------------------------------------------------------------- #
 seas_list <- plyr::llply(.data = 1:25,.fun = function(y){
   ...
   seas_fore_df_stsmolt_daily_full <- do.call(rbind,lapply(X = seasonal_forecast_list_dfs_stsmolt_daily_stoc,
                                                   FUN = function(x){x[[y]]}))
# which(is.na(seas_fore_df_stsmolt_daily_full$rain_mm_1day))
 seas_fore_df_stsmolt_daily <- seas_fore_df_stsmolt_daily_full[seas_fore_df_stsmolt_daily_full$salmonid_year <= 2019,]
#any(is.na(seas_fore_df_stsmolt_daily[[pred_or_sim]]))
 # 3. use existing function to calculate quantile trends in observed data
 seas_fore_df_stsmolt_daily_tercile_data_new <- fishcastr::boxplot_phenology(bio_year = seas_fore_df_stsmolt_daily$salmonid_year,
                                                            counts = seas_fore_df_stsmolt_daily[[pred_or_sim]],
                                                            yday = seas_fore_df_stsmolt_daily$salmonid_yday,
                                                            dates = seas_fore_df_stsmolt_daily$date,
                                                            method = stat,
...
#seasonal_forecast_predictions = seas_list
 ...
 # plot tercile plot -----
  png(file = paste0(dirName,"Fig5b_SEAS5_trout_",type_of_pred,"_",stat,".png"), width=2100, height=2200, res=300)
terciles_summary_stat_plot <-
  fishcastr::tercile_plot_summary_stats(
    seasonal_forecast_predictions = tercile_anoms_sf_ensemble,
    observed_anomalies = obs,
...
saveRDS(object = sum_stats_list_subset,
        file = paste0(dirName,"Fig5b_SEAS5_trout_tercile_data.rds"))
...
dirName <- paste0(dir.data, "/", sep = "", collapse = NULL)
#dir.create(dirName, showWarnings = TRUE, recursive = TRUE, mode = "0777")
write.table(x = windows_op_table, file = paste0(dirName,"windows_op_SEAS5_trout.csv"), row.names = TRUE, sep = ",",col.names = FALSE)
  
# --------------------------------------------------------------------------------------------------#
# PLOT ALL SIMULATIONS (uncomment if required) ----
# --------------------------------------------------------------------------------------------------#
# dirName <- paste0(getwd(),"/vignettes/vignette_figures", "/Fig6", "/trout/year_mem_plot/", sep = "",
#                   collapse = NULL)
# dir.create(dirName, showWarnings = FALSE, mode = "0777")
# 
# type_of_pred <- ifelse(pred_or_sim == "forecast_sim_counts","sim","pred")
...
# --------------------------------------------------------------------------------------------------#
# seas_fore_df_stsmolt_daily <- sum_stats_list$mean$seas_fore_df_stsmolt_daily_full_obs[sum_stats_list$mean$seas_fore_df_stsmolt_daily_full_obs$salmonid_year <= 2019,]
# 
...
#  plot_hist_context_op_forecast_sumstat_poly(seasonal_reforecasts_df = seas_fore_df_stsmolt_daily,
...
```{r figure-5b-trout-result-tab-trout-1, out.width="70%", eval = TRUE, fig.align = 'center', echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                                   "/vignette_figures/Fig5b/trout/")
res_tab <- read.table(file = paste0(dirName,"windows_op_SEAS5_trout.csv"), sep = ",",
                      stringsAsFactors = FALSE)
colnames(res_tab) <- res_tab[1,]
res_tab <- res_tab[-1,]

tab <- knitr::kable(res_tab,row.names = FALSE, format = "html", caption = "(ref:figure-5b-trout-result-tab-trout-1)",
                    booktabs = TRUE)
...
(ref:figure-5b-trout-result-tab-trout-1) *Receiver Operating Characteristic Skill Scores (ROCSS) for tercile based anomalies of migration timing summary statistics under ECMWF SEAS5 forcing.*

```{r figure-5b-trout-plots-trout-1, out.width="70%", eval=FALSE, warning=FALSE, fig.align = 'center', echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                  "/vignette_figures/Fig5b/trout/")

# ---------------------------------------------------------------------------------------------- #
# 5th ----
image <- magick::image_read(paste0(dirName,"Fig5b_SEAS5_trout_sim_5th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5b_SEAS5_trout_sim_5th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 25th ----
image <- magick::image_read(paste0(dirName,"Fig5b_SEAS5_trout_sim_25th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5b_SEAS5_trout_sim_25th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 33rd ----
image <- magick::image_read(paste0(dirName,"Fig5b_SEAS5_trout_sim_33rd.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5b_SEAS5_trout_sim_33rd_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 50th ----
image <- magick::image_read(paste0(dirName,"Fig5b_SEAS5_trout_sim_50th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5b_SEAS5_trout_sim_50th_crop.png"))
...
image_cropped2 <- magick::image_crop(image_cropped, "2300x400+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5b_SEAS5_trout_sim_50th_crop_axis.png"))

# ---------------------------------------------------------------------------------------------- #
# 66th ----
image <- magick::image_read(paste0(dirName,"Fig5b_SEAS5_trout_sim_66th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5b_SEAS5_trout_sim_66th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 75th ----
image <- magick::image_read(paste0(dirName,"Fig5b_SEAS5_trout_sim_75th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5b_SEAS5_trout_sim_75th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 95th ----
image <- magick::image_read(paste0(dirName,"Fig5b_SEAS5_trout_sim_95th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5b_SEAS5_trout_sim_95th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# MEAN ----
image <- magick::image_read(paste0(dirName,"Fig5b_SEAS5_trout_sim_mean.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x400+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5b_SEAS5_trout_sim_mean_crop.png"))
...
Fig5a_ERA5 <- multipanelfigure::multi_panel_figure(columns = 1, width = 132, height = c(24.3,24.3,24.3,24.3,24.3,24.3,24.3,31.0,28.0), unit = "mm") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5b_SEAS5_trout_sim_5th_crop.png"), row = 1,  column = 1,scaling = "shrink", panel_clip = "on",label = "5th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5b_SEAS5_trout_sim_25th_crop.png"), row = 2,  column = 1,scaling = "shrink", panel_clip = "on",label = "25th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5b_SEAS5_trout_sim_33rd_crop.png"), row = 3,  column = 1,scaling = "shrink", panel_clip = "on",label = "33rd") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5b_SEAS5_trout_sim_50th_crop.png"), row = 4,  column = 1,scaling = "shrink", panel_clip = "on",label = "50th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5b_SEAS5_trout_sim_66th_crop.png"), row = 5,  column = 1,scaling = "shrink", panel_clip = "on",label = "66th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5b_SEAS5_trout_sim_75th_crop.png"), row = 6,  column = 1,scaling = "shrink", panel_clip = "on",label = "75th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5b_SEAS5_trout_sim_95th_crop.png"), row = 7,  column = 1,scaling = "shrink", panel_clip = "on",label = "95th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5b_SEAS5_trout_sim_mean_crop.png"), row = 8,  column = 1,scaling = "shrink", panel_clip = "on",label = "mean") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5b_legcrop.png"), row = 9,  column = 1,scaling = "shrink", panel_clip = "on",label = "")
Fig5a_ERA5
invisible(dev.off())
```

```{r figure-5b-trout-plots-trout-2, out.width="70%", eval = TRUE, warning=FALSE, fig.align = 'center',fig.cap = "(ref:figure-5b-trout-plots-trout-2) ", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                  "/vignette_figures/Fig5b/trout/")
knitr::include_graphics(path = paste0(dirName,"Fig5b_SEAS5.png"))
```

(ref:figure-5b-trout-plots-trout-2) *Tercile plots illustrating the model chain's propensity to correctly predict anomaly categories during the retrospective period (1993 - 2019) using ECMWF's SEAS5 as forcing data.*
