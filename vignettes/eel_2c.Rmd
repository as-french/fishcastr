---
title: "An example of seasonal forecast development for European silver eel: Part II of IV"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteEncoding{UTF-8}
  %\VignetteIndexEntry{An example of seasonal forecast development for European silver eel: Part II of IV}
...
This document outlines the calibration and checking of the correlative models for European silver eel (*Anguilla anguilla*) presented in the accompanying manuscript. Specifically, this vignette describes glmmTMB model structure specification, fitting and checking. A separate vignette, "eel_1abc_2ab", describes the model chain elements and derivations of model explanatory variables such as `fishcastr::data_Feeagh_discharge`.
...
```{r model_eel_counts_II_of_IV_fit_model,eval = FALSE}
# ---------------------------------------------------------------------------------------------------------- #
# LOAD COUNT AND PREDICTOR DATA ----
# ---------------------------------------------------------------------------------------------------------- #
data_seel_enviro_1981_2019 <- readRDS(paste0(system.file("vignettes",
                                                    package = "fishcastr"),
...
ifelse(all(param_diffs <= 0.00005), TRUE,FALSE)
 ...
 # extract equation in Tex form
#devtools::install_github("datalorax/equatiomatic")
lr <- lme4::lmer(seel ~ (ln_migration_prep + ln_discharge + moonlight_exposure + Tw_mod + Delta_temp)^2 + (1|eel_year), data=Train_data_scaled_centred_train)
...
dirName <- paste0(system.file("vignettes", package = "fishcastr"),"/vignette_figures/FigS5/eel/")
dir.create(dirName, showWarnings = TRUE, mode = "0777")
...
```{r model_eel_counts_II_of_IV_check_influence_I,eval = FALSE}
# -------------------------------------------------------------------------------------------------- #
# INFLUENTIAL YEARS
...
```{r model_eel_counts_II_of_IV_check_influence_II,eval = FALSE}
library(glmmTMB)
library(car)
...
```{r model_eel_counts_II_of_IV_check_influence_III,eval = FALSE}
chosen_model_influence <- readRDS(paste0(system.file("vignettes",
                                 package = "fishcastr"),
                     "/vignette_data/chosen_model_influence.rds"))
...
```{r model_eel_counts_II_of_IV_check_influence_IV,eval = FALSE}
...
```{r model_eel_counts_II_of_IV_plot_interactions,eval = FALSE}
...
# })
# 
# dirName <- paste0(getwd(),"/vignette_figures", "/FigS5", "/", sep = "", collapse = NULL)
...
```{r model_eel_counts_II_of_IV_plausible_glmmTMB,eval = FALSE}
# -------------------------------------------------------------------------------------------------- #
...
```{r model_eel_counts_II_of_IV_fit_loo,eval = FALSE}            
# ---------------------------------------------------------------------------------------------------------- #
# LOAD COUNT AND PREDICTOR DATA ----
# ---------------------------------------------------------------------------------------------------------- #
data_seel_enviro_1981_2019 <- readRDS(paste0(system.file("vignettes",
...
```{r model_eel_counts_II_of_IV_fit_operational,eval = FALSE}
# ---------------------------------------------------------------------------------------------------------- #
# LOAD COUNT AND PREDICTOR DATA ----
# ---------------------------------------------------------------------------------------------------------- #
data_seel_enviro_1981_2019 <- readRDS(paste0(system.file("vignettes",
...
# Create vignette sub directories ------------------------------------------------------
dirName <- paste0(system.file("vignettes", package = "fishcastr"),"/vignette_figures/FigS5/eel/op_model/")
