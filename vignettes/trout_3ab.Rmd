---
title: "An example of seasonal forecast development for anadromous brown trout smolts: Part III of IV"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteEncoding{UTF-8}
  %\VignetteIndexEntry{An example of seasonal forecast development for anadromous brown trout smolts: Part III of IV}
...
mTrain_genpois_list[(1 + length(model_list_1of2)):(length(model_list_1of2)+length(model_list_2of2))] <- model_list_2of2
 ...
 # add 2019 data
 mTrain_genpois_trout_2019 <- model_list_1of2 <- readRDS(paste0(system.file("vignettes", package = "fishcastr"),
                                    "/vignette_data/mTrain_genpois_trout_2019.rds"))
 pP_params_trout_2019 <- model_list_1of2 <- readRDS(paste0(system.file("vignettes", package = "fishcastr"),
...
data_stsmolt_enviro_1981_2019 <- readRDS(paste0(system.file("vignettes", package = "fishcastr"),
...
seasonal_forecast_list_dfs_stsmolt_daily_stoc <- readRDS(paste0(system.file("vignettes", package = "fishcastr"),
                                   "/vignette_data/SF_trout_ideal_loo_cv_23456.rds"))
...
dirName <- paste0(dir.data, "/", sep = "", collapse = NULL)
#dir.create(dirName, showWarnings = TRUE, recursive = TRUE, mode = "0777")
write.table(x = windows_op_table, file = paste0(dirName,"windows_op_trout_ideal.csv"), row.names = TRUE, sep = ",",col.names = FALSE)
...
# load predictions in list format
seasonal_forecast_list_dfs_stsmolt_daily_stoc <- readRDS(file = paste0(getwd(),"/vignettes/vignette_data\\SF_trout_loo_cv_23456.rds"))
#seasonal_forecast_list_dfs_stsmolt_daily_stoc <- readRDS(file = paste0(getwd(),"/vignettes/vignette_data\\SF_trout_loo_cv_2021.rds"))

forecast_op_year <- 2019

#seasonal_forecast_list_dfs_stsmolt_daily_stoc_new
...
#dev.new()
seas_fore_df_stsmolt_daily_tercile_data <- boxplot_phenology(bio_year = seas_fore_df_stsmolt_daily$salmonid_year,
                                                            counts = seas_fore_df_stsmolt_daily$stsmolt,
                                                            yday = seas_fore_df_stsmolt_daily$salmonid_yday,
                                                            dates = seas_fore_df_stsmolt_daily$date,
                                                            method = stat,
...
#any(is.na(seas_fore_df_stsmolt_daily[[pred_or_sim]]))
 # 3. use existing function to calculate quantile trends in observed data
 seas_fore_df_stsmolt_daily_tercile_data_new <- boxplot_phenology(bio_year = seas_fore_df_stsmolt_daily$salmonid_year,
                                                            counts = seas_fore_df_stsmolt_daily[[pred_or_sim]],
                                                            yday = seas_fore_df_stsmolt_daily$salmonid_yday,
                                                            dates = seas_fore_df_stsmolt_daily$date,
                                                            method = stat,
...
# TERCILE PLOT ----
# ------------------------------------------------------------------------------------------------------------- #
dirName <- paste0(getwd(),"/vignettes/vignette_figures", "/Fig5b", "/trout/", sep = "",
                  collapse = NULL)
dir.create(dirName, showWarnings = FALSE, mode = "0777")
...
#seasonal_forecast_predictions = seas_list
 ...
 # plot tercile plot -----
  png(file = paste0(dirName,"Fig6a_SF21_",type_of_pred,"_",stat,".png"), width=2100, height=2200, res=300)
terciles_summary_stat_plot <-
...
windows_op_table <- t(data.frame("statistic" = sapply(strsplit(df$.id, "\\."),"[",1),
  "early" = paste0(df$early_ROC.score,ifelse(df$early_ROC.significant_0_05 ==TRUE,"*","")),
                  ...
                  "late" = paste0(df$late_ROC.score,ifelse(df$late_ROC.significant_0_05 ==TRUE,"*",""))))
# export table
dir.data <-  paste0(getwd(),'/vignettes/vignette_figures/Fig5b/trout')
dirName <- paste0(dir.data, "/", sep = "", collapse = NULL)
#dir.create(dirName, showWarnings = TRUE, recursive = TRUE, mode = "0777")
write.table(x = windows_op_table, file = paste0(dirName,"windows_op_trout.csv"), row.names = TRUE, sep = ",",col.names = FALSE)
