---
title: 'Appendix S2: A seasonal forecasting workflow for the seaward migrations of Atlantic salmon smolts'
subtitle: |
...
vignette: |
  ...
  %\VignetteIndexEntry{Appendix S2: A seasonal forecasting workflow for the seaward migrations of Atlantic salmon smolts}
...
This appendix is included alongside the manuscript to complement descriptions of data included the manuscript and provide model validation plots specifically related to a model chain for forecasting the timing of Atlantic salmon smolt (*Salmo salar* (Linnaeus, 1758)) migration. See **Appendix S1** for aspects of the seasonal forecasting workflow, which are shared among species; namely:
...
Here, we present complete seaward migrating Atlantic salmon smolt counts recorded between 1981 and 2019 in Burrishoole. **Figure \@ref(fig:catchment-specific-data-1-salmon)** illustrates the width of the temporal window (~60 consecutive calendar days) during which downstream migrating salmon smolts are expected in the Burrishoole fish traps. Note the moderate dispersion of annual counts relative to the phenology curve described by the 1981 - 2019 average daily count.

```{r catchment-specific-data-fish, eval=FALSE, echo=FALSE}
# ---------------------------------------------------------------------------------------------------------- #
# FISH COUNTS ----
# ---------------------------------------------------------------------------------------------------------- #
...
mtext(side = 2,text = expression(italic("S. salar ")~smolt~count),outer = TRUE,line = 0)
...
```{r catchment-specific-data-1-salmon, out.width="70%", eval = TRUE, fig.align = 'center', fig.cap = '(ref:catchment-specific-data-1-salmon)', echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/salmon_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"catchment_specific_data-1-salmon.png"))
```

(ref:catchment-specific-data-1-salmon) *Migration phenology curves (relative to 1981 - 2019 average) described by the combined daily counts of Atlantic salmon (*Salmo salar*) recorded leaving the two freshwater outflows from Lough Feeagh and entering a tidal lagoon at Burrishoole, Ireland.*

*Stratifying salmon smolt census data into forecast years*

For salmon smolts, we stratified the census into forecast years (salmonid years) by defining the winter solstice as the first day of each forecast year. This was based on concurrent onset of smoltification with increasing photoperiod [@mccormick_low_2000; @strand_photoperiod_2018].

*Observed migration timing anomalies and phenology curves*

To clarify context regarding anomaly definitions, we are skipping ahead to one of the outputs of workflow step 3. In **Figure \@ref(fig:catchment-specific-data-1b-salmon)**, we present observed (tercile based) anomalies for the mean date of migration of Atlantic salmon smolts in Burrishoole (NOTE - for comparison with later seasonal forecast anomalies, computations of migration summary statistics are based solely on fish counts during forecast year "target season"; i.e., March to August for salmon smolts). These data are not detrended, although the fishcastr package facilitates *linear* detrending of terciles; i.e., adjusting the 1/3 and 2/3 percentiles positions according to a fitted linear regression. In the presence of multidecadal trends in migration timing, linear detrending can be useful for assessing a seasonal forecast system's propensity to discriminate between occurrence and non-occurrence of anomalies, when the definition of anomalies for each year are adjusted relative to expectations given "contemporary conditions". Detrending is also useful for exploring phenotypic plasticity [@iler_detrending_2017] by answering questions such as: *Is inter-annual variability in timing anomalies more or less variable during 2000-2010 that variability during 1980-2000?*; i.e., *Do Atlantic salmon smolts have a greater or lesser propensity to adjust their migration timing to match seasonal conditions?*. This question assumes inter-annual variability in seasonal climate is consistent throughout the time series, although quantification of inter-annual variability in seasonal climate for selected windows could be used to adjust for any differences.

```{r observed-anomalies-atlantic-salmon, eval = FALSE, echo = FALSE}
# ---------------------------------------------------------------------------------------------------------- #
...
# salmon smolts are counted primarily from April through August
data_ssmolt$month <- lubridate::month(data_ssmolt$date)
tapply(data_ssmolt$ssmolt, data_ssmolt$month, sum)

# merge data and subset to target season
data_counts <- merge(data_ssmolt,data_ssmolt_forecast_years, by = "date", all.x = TRUE)
...
data_counts_salmon <- data_counts[data_counts$salmonid_year %in% c(1981:2019),]
data_counts_salmon$month <- lubridate::month(data_counts_salmon$date)
data_counts_salmon_target_s <- data_counts_salmon
data_counts_salmon_target_s$ssmolt[data_counts_salmon_target_s$month %in% c(1:2,9:12)] <- 0
...
dirName <- paste0(system.file("extdata",package = "fishcastr"),
                      "/salmon_wflow_figs/", sep = "", collapse = NULL)
dir.create(dirName, showWarnings = TRUE, mode = "0777")

png(file = paste0(dirName,"catchment_specific_data-1b-salmon.png"), width=4500, height=6500, res=150)
salmon_anoms <- fishcastr::ridgeline_plot_phenology(bio_year = data_counts_salmon_target_s$salmonid_year,
                                 counts = data_counts_salmon_target_s$ssmolt,
                                 yday = data_counts_salmon_target_s$salmonid_yday,
                                 dates = data_counts_salmon_target_s$date,
                                 method = "mean",
...
image <- magick::image_read(paste0(dirName,"catchment_specific_data-1b-salmon.png"))
image_trimmed <- magick::image_trim(image) # trim white space
magick::image_write(image_trimmed, paste0(dirName,"catchment_specific_data-1b-salmon_trim.png"))
```

```{r catchment-specific-data-1b-salmon, out.width="70%", eval = TRUE, fig.align = 'center', fig.cap = "(ref:catchment-specific-data-1b-salmon)", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/salmon_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"catchment_specific_data-1b-salmon_trim.png"))
```

(ref:catchment-specific-data-1b-salmon) *Migration phenology curves and migration timing anomalies (of mean forecast year day) described by the combined daily counts of Atlantic salmon (*S. salar*) recorded leaving the two freshwater outflows from Lough Feeagh and entering a tidal lagoon at Burrishoole, Ireland.*
...
legend("bottomright", legend = c("Water temperature","2004 - 2019 mean daily water temperature\nand 95 percentile range"),col = c("red",col_alpha_black), pch = c(NA,15),lty = c(1,NULL),bty = "n",inset = c(0.1,-1.8), xpd = NA, cex = 1.5)
dev.off()
```

```{r catchment-specific-data-2-salmon, out.width="70%", eval=FALSE, fig.align = 'center',fig.cap = "\\textit{Lough Feeagh AWQMS recorded water temperature at 2m depth (relative to 2004 - 2019 average).}", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/salmon_wflow_figs/")
...
```{r catchment-specific-data-3-salmon, out.width="70%", eval=FALSE, fig.align = 'center', fig.cap = "\\textit{Rating curve estimated catchment discharge from the combined outflows of Lough Feeagh (relative to 1981 - 2019 average).}", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/salmon_wflow_figs/")
...
```{r lunar-solar-data-2-salmon, out.width="70%", eval=FALSE, fig.align = 'center',fig.cap = "\\textit{(Clear-sky) Moonlight exposure estimated for the Burrishoole catchment calculated using suncalc::getSunlightTimes, suncalc::getMoonIllumination and suncalc::getMoonPosition.}", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/salmon_wflow_figs/")
...
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/salmon_wflow_figs/")
png(filename = paste0(dirName,"access_climate_data-3-salmon_",x,"_SF.png"), height = 1700, width = 1700, res =300)
...
```{r access-climate-data-1-salmon-tas, out.width="70%", eval=FALSE, fig.align = 'center',fig.cap = "\\textit{Recorded mean daily air temperature at the manual meteorological station in Burrishoole (relative to 1981 - 2019 average)}", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/salmon_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"access_climate_data-1-salmon_tas_manual.png"))
```

```{r access-climate-data-1-salmon-pr, out.width="70%", eval=FALSE, fig.align = 'center',fig.cap = "\\textit{Recorded daily total precipitation at the manual meteorological station in Burrishoole (relative to 1981 - 2019 average)}", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/salmon_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"access_climate_data-1-salmon_pr_manual.png"))
```

```{r access-climate-data-1-salmon-petH, out.width="70%", eval=FALSE, fig.align = 'center',fig.cap = "\\textit{Estimated daily potential evapotranspiration in Burrishoole estimated using the Hargreaves-Samani equation (relative to 1981 - 2019 average).}", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/salmon_wflow_figs/")
...
For each retrospective seasonal salmon smolt migration forecast, we append two years (antecedent) bias adjusted ERA5 climate reanalysis up to the end of January to SEAS5 seasonal climate forecasts from February to August - hence the cessation of ERA5 data in early-2019 illustrated in **Figure \@ref(fig:access-climate-data-2-salmon-tas)**. The data presented here are bias adjusted relative to local meteorological observations; therefore, the long run average climate illustrated by shading of **Figure \@ref(fig:access-climate-data-2-salmon-tas)** are consistent with meteorological observation long run averages.

```{r access-climate-data-2-salmon-tas, out.width="70%", eval = TRUE, fig.align = 'center', fig.cap = "(ref:access-climate-data-2-salmon-tas)",echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/salmon_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"access_climate_data-2-salmon_tas_reanalysis.png"))
```

(ref:access-climate-data-2-salmon-tas) *ECMWF ERA5 modelled mean daily air temperature at 2m (calculated from hourly resolution ERA5) in Burrishoole (relative to 1981 - 2019 average).*

```{r access-climate-data-2-salmon-pr, out.width="70%", eval=FALSE, fig.align = 'center', fig.cap = "\\textit{ECMWF ERA5 modelled daily total precipitation in Burrishoole (relative to 1981 - 2019 average).}",echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/salmon_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"access_climate_data-2-salmon_pr_reanalysis.png"))
```

```{r access-climate-data-2-salmon-petH, out.width="70%", eval=FALSE, fig.align = 'center', fig.cap = "\\textit{Hargreaves-Samani equation estimated daily potential evapotranspiration calculated from ECMWF ERA5 modelled daily minimum and maximum temperatures (extracted from hourly resolution ERA5) in Burrishoole (relative to 1981 - 2019 average).}",echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/salmon_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"access_climate_data-2-salmon_petH_reanalysis.png"))
```

### *Seasonal climate forecast*

We accessed SEAS5 seasonal climate forecasts initialised on February 1 from which we retained data up to August 31. **Figure \@ref(fig:access-climate-data-3-salmon-tas)** illustrates ensemble spread relative to long run average (1993 - 2019) ensemble spread; that is, following bias adjustment, long run average ensemble spread (illustrated by shading in **Figure \@ref(fig:access-climate-data-3-salmon-tas)**) is consistent with long run average climate calculated from local meteorological observations.

```{r access-climate-data-3-salmon-tas, out.width="70%", eval = TRUE, fig.align = 'center', fig.cap = "(ref:access-climate-data-3-salmon-tas)",echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/salmon_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"access_climate_data-3-salmon_tas_SF.png"))
```

(ref:access-climate-data-3-salmon-tas) *ECMWF SEAS5 modelled mean daily air temperature at 2m (calculated from 6-hourly resolution SEAS5) in Burrishoole (relative to 1993 - 2019 average).*

```{r access-climate-data-3-salmon-pr, out.width="70%", eval=FALSE, fig.align = 'center', fig.cap = "\\textit{ECMWF SEAS5 modelled daily total precipitation in Burrishoole (relative to 1993 - 2019 average).}",echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/salmon_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"access_climate_data-3-salmon_pr_SF.png"))
```

```{r access-climate-data-3-salmon-petH, out.width="70%", eval=FALSE, fig.align = 'center', fig.cap = "\\textit{Hargreaves-Samani equation estimated daily potential evapotranspiration calculated from ECMWF SEAS modelled daily minimum and maximum temperatures (extracted from 6-hourly resolution SEAS5) in Burrishoole (relative to 1993 - 2019 average).}",echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),"/salmon_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"access_climate_data-3-salmon_petH_SF.png"))
```
...
```{r model-proxy-preparedness-1-salmon, out.width="60%", eval = TRUE, fig.align = 'center',fig.cap = "(ref:model-proxy-preparedness-1-salmon)", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                  "/vignette_figures/migration_prep_models/salmon/")
knitr::include_graphics(path = paste0(dirName,"genpois_salmon.png"))
```

(ref:model-proxy-preparedness-1-salmon) *Fitted exponentially modified Gaussian relationship between photoperiod-weighted degree days and scaled daily counts of Atlantic salmon (*S. salar*) smolts recorded in the freshwater outflow traps of the Burrishoole catchment. Observations are colour-faceted by decade and are illustrated relative to the positive y axis. Simulations from the model fitted to data from 1981 - 1992 are mirrored on the negative y axis.*

```{r model-proxy-preparedness-2-salmon, fig.show = "hold", out.width="40%", eval = TRUE, fig.align = 'center',fig.cap = "(ref:model-proxy-preparedness-2-salmon)", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
...
(ref:model-proxy-preparedness-2-salmon) *Model checking plots for fitted exponentially modified Gaussian relationship between photoperiod-weighted degree days and scaled daily counts of Atlantic salmon (*S. salar*) smolts recorded in the freshwater outflow traps of the Burrishoole catchment.*

## (2c) Model fish counts

### *Calculate predictors*

Prior to fitting glmmTMB models, we pre-processed the raw and model derived catchment environmental data included in `fishcastr`. Pre-processing steps included stratification of salmon forecast years, log transformation of positive skewed predictors (moonlight exposure, discharge and migration preparedness). We included both the log transformed and non-transformed data of these three "likely candidates" in the initial modelling dataset, as it was not clear form the outset whether transformations would improve statistical fit and or computation of the fitting procedure. The inclusion of log transformed vs. non-transformed variables varied among species.
...
Residual plots for models trained using data for 1981 - 1992 were used to refine model structure for salmon smolts (**Figure \@ref(fig:model-salmon-counts-1981-1992-fit-salmon-1)**). Residual plots for models trained using data for 1981 - 2018 were used to examine the extent to which the 1981- 1992 model structure remained appropriate for contemporary predictions (**Figure \@ref(fig:model-salmon-counts-2019-fit-operational-salmon-1)**).

```{r model-salmon-counts-1981-1992-fit,eval = FALSE, echo = FALSE}
# ---------------------------------------------------------------------------------------------------------- #
...
```{r model-salmon-counts-1981-1992-fit-salmon-1, fig.show = "hold", out.width="30%", eval = TRUE, fig.align = 'center', fig.cap = "(ref:model-salmon-counts-1981-1992-fit-salmon-1)", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
...
(ref:model-salmon-counts-1981-1992-fit-salmon-1) *Model validation plots for daily fish count model trained on data from 1981 - 1992.*

<!-- ### Fit refined model to data using leave-one-year-out approach -->

```{r model-salmon-counts-1981-2018-fit-loo,eval = FALSE, echo = FALSE}
# ---------------------------------------------------------------------------------------------------------- #
# LOAD COUNT AND PREDICTOR DATA ----
# ---------------------------------------------------------------------------------------------------------- #
data_ssmolt_enviro_1981_2019 <- readRDS(paste0(system.file("extdata",
...
```{r model-salmon-counts-2019-fit-operational,eval = FALSE, echo = FALSE}
# ---------------------------------------------------------------------------------------------------------- #
# LOAD COUNT AND PREDICTOR DATA ----
# ---------------------------------------------------------------------------------------------------------- #
data_ssmolt_enviro_1981_2019 <- readRDS(paste0(system.file("extdata",
...
```{r model-salmon-counts-2019-fit-operational-salmon-1, fig.show = "hold", out.width="30%", eval = TRUE, fig.align = 'center', fig.cap = "(ref:model-salmon-counts-2019-fit-operational-salmon-1)", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                  "/vignette_figures/count_model_checks/salmon/op_model/")
...
(ref:model-salmon-counts-2019-fit-operational-salmon-1) *Model validation plots for daily fish count model trained on data from 1981 - 2018.*
...
Prior to evaluating seasonal forecast skill for the salmon model chain under ERA5 and SEAS5 forcing, we can evaluate forecast skill for specific SEAS5 meteorological forcing variables themselves. Consistent with convention, we specify a lead time (e.g., one month; February) and target season period (e.g., March to May) (**Figures \@ref(fig:seas-forecast-skill-SEAS5-spring-salmon-tas)**, **\@ref(fig:seas-forecast-skill-SEAS5-spring-salmon-pr)** and **\@ref(fig:seas-forecast-skill-SEAS5-spring-salmon-petH)**).

```{r seas-forecast-skill-SEAS5-spring-salmon, eval = FALSE, echo = FALSE}
# LOAD ERA 5 RDATA
...
cofinogramdat <-
  lapply(variables, function(i) {
    filename = paste0(system.file("extdata", package = "fishcastr"),"/salmon_wflow_figs/SEAS5_",paste0(target_season, collapse = "_"),"_",paste0(i, collapse = "_"),".png")
...
saveRDS(object = cofinogramdat,
        file = paste0(system.file("extdata", package = "fishcastr"),
                                              "/salmon_wflow_figs/climate_terciles_",
...
image <- magick::image_read(paste0(system.file("extdata",package = "fishcastr"),
                      "/salmon_wflow_figs/SEAS5_3_4_5_tas.png"))
image_trimmed <- magick::image_trim(image) # trim white space
magick::image_write(image_trimmed, paste0(system.file("extdata",package = "fishcastr"),
                      "/salmon_wflow_figs/SEAS5_3_4_5_tas.png"))

image <- magick::image_read(paste0(system.file("extdata",package = "fishcastr"),
                      "/salmon_wflow_figs/SEAS5_3_4_5_pr.png"))
image_trimmed <- magick::image_trim(image) # trim white space
magick::image_write(image_trimmed, paste0(system.file("extdata",package = "fishcastr"),
                      "/salmon_wflow_figs/SEAS5_3_4_5_pr.png"))

image <- magick::image_read(paste0(system.file("extdata",package = "fishcastr"),
                      "/salmon_wflow_figs/SEAS5_3_4_5_petH.png"))
image_trimmed <- magick::image_trim(image) # trim white space
magick::image_write(image_trimmed, paste0(system.file("extdata",package = "fishcastr"),
                      "/salmon_wflow_figs/SEAS5_3_4_5_petH.png"))
```

```{r seas-forecast-skill-SEAS5-spring-salmon-tas, out.width="70%", eval = TRUE, fig.align = 'center',fig.cap = "(ref:seas-forecast-skill-SEAS5-spring-salmon-tas)", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                  "/salmon_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"SEAS5_3_4_5_tas.png"))
```

(ref:seas-forecast-skill-SEAS5-spring-salmon-tas) *Predictive performance of SEAS5 seasonal climate forecast for seasonal mean daily mean air temperature during the target season (March to May). Forecasts have a lead time of one month (i.e., initialisation on 1 February).*

```{r seas-forecast-skill-SEAS5-spring-salmon-pr, out.width="70%", eval = TRUE, fig.align = 'center',fig.cap = "(ref:seas-forecast-skill-SEAS5-spring-salmon-pr)", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                  "/salmon_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"SEAS5_3_4_5_pr.png"))
```

(ref:seas-forecast-skill-SEAS5-spring-salmon-pr) *Predictive performance of SEAS5 seasonal climate forecast for seasonal total precipitation during the target season (March to May). Forecasts have a lead time of one month (i.e., initialisation on 1 February).*

```{r seas-forecast-skill-SEAS5-spring-salmon-petH, out.width="70%", eval = TRUE, fig.align = 'center', fig.cap = "(ref:seas-forecast-skill-SEAS5-spring-salmon-petH)",echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                  "/salmon_wflow_figs/")
knitr::include_graphics(path = paste0(dirName,"SEAS5_3_4_5_petH.png"))
```

(ref:seas-forecast-skill-SEAS5-spring-salmon-petH) *Predictive performance of SEAS5 seasonal climate forecast for seasonal total potential evapotranspiration during the target season (March to May). Forecasts have a lead time of one month (i.e., initialisation on 1 February).*
...
```{r figure-5a-salmon-methods, warning = FALSE, eval = FALSE, echo = FALSE}
...
fish_counts_ERA5 <- fishcastr::plot_ensemble_data(seasonal_ensemble = seasonal_forecast_list_dfs_ssmolt_daily_stoc_era5,
...
hydro_ERA5 <- lapply(seasonal_forecast_list_dfs_ssmolt_daily_stoc_era5, function(x){
...
wtemp_ERA5 <- lapply(seasonal_forecast_list_dfs_ssmolt_daily_stoc_era5, function(x){
 res <- lapply(x, function(y){
...
```{r era5-model-chain-checks-salmon-plot-counts, fig.align = "center", out.width="70%", eval = TRUE,fig.cap = "(ref:era5-model-chain-checks-salmon-plot-counts)", echo = FALSE}
knitr::include_graphics(paste0(system.file("extdata", package = "fishcastr"),
                               "/salmon_wflow_figs/ERA5_forecast_sim_counts.png"))
```

(ref:era5-model-chain-checks-salmon-plot-counts) *Daily resolution Atlantic salmon (*S. salar*) smolt simulated counts recorded at Burrishoole during 1981 to 2019 during the seasonal forecast target season (March to July). Grey filled bars illustrate observations, while the 95% prediction intervals of simulated counts generated using known (held-out) model chain forcing data (ECMWF's ERA5 climate reanalysis) are illustrated by green polygons. The green line running through the polygon is the daily median simulated count. Daily median and 95% prediction intervals were calculated from 25 simulations per year. Note the different y-axis scales among plots.*


Further exploration of model chain predictions for hydrology (**Figure \@ref(fig:era5-model-chain-checks-salmon-plot-discharge)**) and water temperature (**Figure \@ref(fig:era5-model-chain-checks-salmon-plot-water-temp)**) are useful for identifying any systematic biases in seasonal prediction that are less clear when validating the model as a whole (e.g., observation vs. prediction plots and using KGE and rmse statistics).

```{r era5-model-chain-checks-salmon-plot-discharge, fig.align = "center", out.width="70%", eval = TRUE, fig.cap = "(ref:era5-model-chain-checks-salmon-plot-discharge)",echo = FALSE}
knitr::include_graphics(paste0(system.file("extdata", package = "fishcastr"),
                               "/salmon_wflow_figs/ERA5_discharge.png"))
```

(ref:era5-model-chain-checks-salmon-plot-discharge) *Daily resolution GR4J hydrologic model predictions and concurrent rating curve derived catchment discharge recorded at Burrishoole during 1981 to 2019 during the seasonal forecast antecedent, lead-time and target season for Atlantic salmon (*S. salar*) smolt (December 20/21/22 to July 31). Black lines illustrate rating curve derived discharge, while blue lines illustrate GR4J predictions generated using known (held-out) model chain forcing data (ECMWF's ERA5 climate reanalysis). Note the different y-axis scales among plots.*

```{r era5-model-chain-checks-salmon-plot-water-temp, fig.align = "center", out.width="70%", eval = TRUE, fig.cap = "(ref:era5-model-chain-checks-salmon-plot-water-temp)",echo = FALSE}
knitr::include_graphics(paste0(system.file("extdata", package = "fishcastr"),
                               "/salmon_wflow_figs/ERA5_Tw_mod.png"))
```

(ref:era5-model-chain-checks-salmon-plot-water-temp) *Daily resolution statistical air to water temperature model predictions and concurrent measurements recorded at Burrishoole during 1981 to 2019 during the seasonal forecast antecedent, lead-time and target season for Atlantic salmon (*S. salar*) smolt (December 20/21/22 to July 31). Black lines illustrate observations (1981 to 2003 paper chart recorder in Mill Race; 2004 - 2019 Lough Feeagh AWQMS 2m depth temperature), while red lines illustrate model predictions generated using known (held-out) model chain forcing data (ECMWF's ERA5 climate reanalysis). Note the different y-axis scales among plots.*
...
```{r figure-5b-salmon-methods, warning = FALSE,  fig.height=7, fig.width=7, eval = FALSE, echo = FALSE}
...
The following **Figures \@ref(fig:SEAS5-model-chain-checks-salmon-plot-counts)**, **\@ref(fig:SEAS5-model-chain-checks-salmon-plot-discharge)** and **\@ref(fig:SEAS5-model-chain-checks-salmon-plot-water-temp)** illustrate ensemble spread for simulated variable outputs from model chain components.

```{r SEAS5-model-chain-checks-salmon, warning = FALSE, eval = FALSE, echo = FALSE}
# --------------------------------------------------------------------------------------- #
# LOAD SEASONAL FORECAST DATA (PREDICTIONS/SIMULATIONS/PESUEDO-OBSERVATIONS)
# --------------------------------------------------------------------------------------- #
seasonal_forecast_list_dfs_ssmolt_daily_stoc_SEAS5 <- readRDS(file = paste0(system.file("vignettes", package = "fishcastr"),
                                      "/vignette_data\\SF_salmon_loo_cv_23456.rds"))
...
seasonal_forecast_list_dfs_ssmolt_daily_stoc_SEAS5[[27]] <-
  lapply(seasonal_forecast_list_dfs_ssmolt_daily_stoc_SEAS5[[27]], function(x) {
    NA_dates_2019 <- x$date[which(is.na(x$ssmolt))]
    x$ssmolt[x$date %in% NA_dates_2019] <-
      data_ssmolt$ssmolt[data_ssmolt$date %in% NA_dates_2019]
    return(x)
...
fish_counts_SEAS5 <- fishcastr::plot_ensemble_data(seasonal_ensemble = seasonal_forecast_list_dfs_ssmolt_daily_stoc_SEAS5,
                           pseudo_obs_ensemble = seasonal_forecast_list_dfs_ssmolt_daily_stoc_SEAS5,
                           var_interest = "forecast_sim_counts",
                           obs_count_name = "ssmolt",
                           bio_yday = "salmonid_yday",
                           bio_year = "salmonid_year",
...
hydro_SEAS5 <- lapply(seasonal_forecast_list_dfs_ssmolt_daily_stoc_SEAS5, function(x){
 res <- lapply(x, function(y){
    z <- merge(y,calculated_Q,by = "date",all.x = TRUE)
    return(z)
  })
...
hydrology <- fishcastr::plot_ensemble_data(seasonal_ensemble = hydro_SEAS5,
...
wtemp_SEAS5 <- lapply(seasonal_forecast_list_dfs_ssmolt_daily_stoc_SEAS5, function(x){
 res <- lapply(x, function(y){
    z <- merge(y,water_temp.merge.subset,by = "date",all.x = TRUE)
    return(z)
  })
...
water_temperature <- fishcastr::plot_ensemble_data(seasonal_ensemble = wtemp_SEAS5,
...
```{r SEAS5-model-chain-checks-salmon-plot-counts, fig.align = "center", out.width="70%", eval = TRUE, fig.cap = "(ref:SEAS5-model-chain-checks-salmon-plot-counts)",echo = FALSE}
knitr::include_graphics(paste0(system.file("extdata", package = "fishcastr"),
                               "/salmon_wflow_figs/SEAS5_forecast_sim_counts.png"))
```

(ref:SEAS5-model-chain-checks-salmon-plot-counts) *Daily resolution Atlantic salmon (*S. salar*) smolt simulated counts recorded at Burrishoole during 1993 to 2019 during the seasonal forecast target season (March to July). Grey filled bars illustrate observations, while the 95% prediction intervals of simulated counts generated using seasonal forecast model chain forcing data (ECMWF's SEAS5 seasonal forecast system) are illustrated by green polygons. The green line running through the polygon is the daily median simulated count. Daily median and 95% prediction intervals were calculated from 25 simulations per year. Note the different y-axis scales among plots.*

```{r SEAS5-model-chain-checks-salmon-plot-discharge, fig.align = "center", out.width="70%", eval = TRUE, fig.cap = "(ref:SEAS5-model-chain-checks-salmon-plot-discharge)",echo = FALSE}
knitr::include_graphics(paste0(system.file("extdata", package = "fishcastr"),
                               "/salmon_wflow_figs/SEAS5_discharge.png"))
```

(ref:SEAS5-model-chain-checks-salmon-plot-discharge) *Daily resolution GR4J hydrologic model predictions and concurrent rating curve derived catchment discharge recorded at Burrishoole during 1993 to 2019 during the seasonal forecast antecedent, lead-time and target season for Atlantic salmon (*S. salar*) smolt (December 20/21/22 to July 31). Black lines illustrate rating curve derived discharge, while blue polygons illustrate 95% prediction intervals of GR4J predictions generated using seasonal forecast model chain forcing data (ECMWF's SEAS5 seasonal forecast system). Note the different y-axis scales among plots.*

```{r SEAS5-model-chain-checks-salmon-plot-water-temp, fig.align = "center", out.width="70%", eval = TRUE, fig.cap = "(ref:SEAS5-model-chain-checks-salmon-plot-water-temp)",echo = FALSE}
knitr::include_graphics(paste0(system.file("extdata", package = "fishcastr"),
                               "/salmon_wflow_figs/SEAS5_Tw_mod.png"))
```

(ref:SEAS5-model-chain-checks-salmon-plot-water-temp) *Daily resolution statistical air to water temperature model predictions and concurrent measurements recorded at Burrishoole during 1981 to 2019 during the seasonal forecast antecedent, lead-time and target season for Atlantic salmon (*S. salar*) smolt (December 20/21/22 to July 31). Black lines illustrate observations (1993 to 2003 paper chart recorder in Mill Race; 2004 - 2019 Lough Feeagh AWQMS 2m depth temperature), while red polygons illustrate 95% prediction intervals of model predictions generated using seasonal forecast model chain forcing data (ECMWF's SEAS5 seasonal forecast system). Note the different y-axis scales among plots.*

## (3b) Evaluate retrospective skill

Tabulated skill scores compactly reveal significant skill scores under ERA5 (**Table \@ref(tab:figure-5a-salmon-result-tab-salmon-1)**) and SEAS5 (**Table \@ref(tab:figure-5b-salmon-result-tab-salmon-1)**) forcing for a range of migration timing seasonal summary statistics. Percentile specific tercile plots for seasonal migration predictions under ERA5 (**Figure \@ref(fig:figure-5a-salmon-plots-salmon-2)**) and SEAS5 (**Figure \@ref(fig:figure-5b-salmon-plots-salmon-2)**) forcing reveal year-to-year accuracy of probabilistic predictions.

<!-- ### Known conditions (ECMWF's ERA5) -->

```{r figure-5a-salmon-plots, warning = FALSE,  fig.height=7, fig.width=7, eval = FALSE, echo = FALSE}
...
(ref:figure-5a-salmon-result-tab-salmon-1) *Receiver Operating Characteristic Skill Scores (ROCSS) for tercile based anomalies of migration timing summary statistics under ECMWF ERA5 forcing.*

```{r figure-5a-salmon-result-tab-salmon-1, out.width="70%", eval = TRUE, fig.align = 'center', echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                                   "/vignette_figures/Fig5a/salmon/")
...
tab <- knitr::kable(res_tab,row.names = FALSE, format = "html",
                    caption = "(ref:figure-5a-salmon-result-tab-salmon-1)",
                    booktabs = TRUE)
...
```{r figure-5a-salmon-plots-salmon-1, out.width="70%", eval=FALSE, warning=FALSE, fig.align = 'center', echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                  "/vignette_figures/Fig5a/salmon/")

# ---------------------------------------------------------------------------------------------- #
# 5th ----
image <- magick::image_read(paste0(dirName,"Fig5a_ERA5_salmon_sim_5th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
...
magick::image_write(image_chopped2, paste0(dirName,"Fig5a_ERA5_salmon_sim_5th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 25th ----
image <- magick::image_read(paste0(dirName,"Fig5a_ERA5_salmon_sim_25th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5a_ERA5_salmon_sim_25th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 33rd ----
image <- magick::image_read(paste0(dirName,"Fig5a_ERA5_salmon_sim_33rd.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5a_ERA5_salmon_sim_33rd_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 50th ----
image <- magick::image_read(paste0(dirName,"Fig5a_ERA5_salmon_sim_50th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5a_ERA5_salmon_sim_50th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 66th ----
image <- magick::image_read(paste0(dirName,"Fig5a_ERA5_salmon_sim_66th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5a_ERA5_salmon_sim_66th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 75th ----
image <- magick::image_read(paste0(dirName,"Fig5a_ERA5_salmon_sim_75th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5a_ERA5_salmon_sim_75th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 95th ----
image <- magick::image_read(paste0(dirName,"Fig5a_ERA5_salmon_sim_95th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5a_ERA5_salmon_sim_95th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# mean ----
image <- magick::image_read(paste0(dirName,"Fig5a_ERA5_salmon_sim_mean.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x400+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5a_ERA5_salmon_sim_mean_crop.png"))
...
multipanelfigure::fill_panel(paste0(dirName,"Fig5a_ERA5_salmon_sim_5th_crop.png"), row = 1,  column = 1,scaling = "shrink", panel_clip = "on",label = "5th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5a_ERA5_salmon_sim_25th_crop.png"), row = 2,  column = 1,scaling = "shrink", panel_clip = "on",label = "25th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5a_ERA5_salmon_sim_33rd_crop.png"), row = 3,  column = 1,scaling = "shrink", panel_clip = "on",label = "33rd") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5a_ERA5_salmon_sim_50th_crop.png"), row = 4,  column = 1,scaling = "shrink", panel_clip = "on",label = "50th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5a_ERA5_salmon_sim_66th_crop.png"), row = 5,  column = 1,scaling = "shrink", panel_clip = "on",label = "66th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5a_ERA5_salmon_sim_75th_crop.png"), row = 6,  column = 1,scaling = "shrink", panel_clip = "on",label = "75th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5a_ERA5_salmon_sim_95th_crop.png"), row = 7,  column = 1,scaling = "shrink", panel_clip = "on",label = "95th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5a_ERA5_salmon_sim_mean_crop.png"), row = 8,  column = 1,scaling = "shrink", panel_clip = "on",label = "mean") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5a_legcrop.png"), row = 9,  column = 1,scaling = "shrink", panel_clip = "on",label = "")
Fig5a_ERA5
invisible(dev.off())
```

```{r figure-5a-salmon-plots-salmon-2, out.width="70%", eval = TRUE, warning=FALSE, fig.align = 'center',fig.cap = "(ref:figure-5a-salmon-plots-salmon-2)", echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                  "/vignette_figures/Fig5a/salmon/")
knitr::include_graphics(path = paste0(dirName,"Fig5a_ERA5.png"))
```

(ref:figure-5a-salmon-plots-salmon-2) *Tercile plots illustrating the model chain's propensity to correctly predict anomaly categories during the retrospective period (1981 - 2019) using ECMWF's ERA5 as forcing data.*

<!-- ### *Seasonal forecast (ECMWF's SEAS5)* -->

```{r figure-5b-salmon-plots, warning = FALSE,  fig.height=7, fig.width=7, eval = FALSE, echo = FALSE}
library(fishcastr)
...
(ref:figure-5b-salmon-result-tab-salmon-1) *Receiver Operating Characteristic Skill Scores (ROCSS) for tercile based anomalies of migration timing summary statistics under ECMWF SEAS5 forcing.*

```{r figure-5b-salmon-result-tab-salmon-1, out.width="70%", eval = TRUE, fig.align = 'center', echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                                   "/vignette_figures/Fig5b/salmon/")
...
tab <- knitr::kable(res_tab,row.names = FALSE, format = "html", caption = "(ref:figure-5b-salmon-result-tab-salmon-1)",
                    booktabs = TRUE)
...
```{r figure-5b-salmon-plots-salmon-1, out.width="70%", eval=FALSE, warning=FALSE, fig.align = 'center', echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                  "/vignette_figures/Fig5b/salmon/")

# ---------------------------------------------------------------------------------------------- #
# 5th ----
image <- magick::image_read(paste0(dirName,"Fig5b_SEAS5_salmon_sim_5th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5b_SEAS5_salmon_sim_5th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 25th ----
image <- magick::image_read(paste0(dirName,"Fig5b_SEAS5_salmon_sim_25th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5b_SEAS5_salmon_sim_25th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 33rd ----
image <- magick::image_read(paste0(dirName,"Fig5b_SEAS5_salmon_sim_33rd.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5b_SEAS5_salmon_sim_33rd_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 50th ----
image <- magick::image_read(paste0(dirName,"Fig5b_SEAS5_salmon_sim_50th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5b_SEAS5_salmon_sim_50th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 66th ----
image <- magick::image_read(paste0(dirName,"Fig5b_SEAS5_salmon_sim_66th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5b_SEAS5_salmon_sim_66th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 75th ----
image <- magick::image_read(paste0(dirName,"Fig5b_SEAS5_salmon_sim_75th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5b_SEAS5_salmon_sim_75th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# 95th ----
image <- magick::image_read(paste0(dirName,"Fig5b_SEAS5_salmon_sim_95th.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x300+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5b_SEAS5_salmon_sim_95th_crop.png"))

# ---------------------------------------------------------------------------------------------- #
# MEAN ----
image <- magick::image_read(paste0(dirName,"Fig5b_SEAS5_salmon_sim_mean.png"))
image_trimmed <- magick::image_trim(image) # trim white space
image_cropped <- magick::image_crop(image_trimmed, "2300x770+0x0", gravity = c("southwest")) 
image_cropped2 <- magick::image_crop(image_cropped, "2300x400+0x0", gravity = c("northwest")) 
image_chopped2 <- magick::image_trim(image_cropped2)
magick::image_write(image_chopped2, paste0(dirName,"Fig5b_SEAS5_salmon_sim_mean_crop.png"))
...
Fig5a_ERA5 <- multipanelfigure::multi_panel_figure(columns = 1, width = 132, height = c(24.3,24.3,24.3,24.3,24.3,24.3,24.3,31.0,28.0), unit = "mm") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5b_SEAS5_salmon_sim_5th_crop.png"), row = 1,  column = 1,scaling = "shrink", panel_clip = "on",label = "5th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5b_SEAS5_salmon_sim_25th_crop.png"), row = 2,  column = 1,scaling = "shrink", panel_clip = "on",label = "25th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5b_SEAS5_salmon_sim_33rd_crop.png"), row = 3,  column = 1,scaling = "shrink", panel_clip = "on",label = "33rd") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5b_SEAS5_salmon_sim_50th_crop.png"), row = 4,  column = 1,scaling = "shrink", panel_clip = "on",label = "50th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5b_SEAS5_salmon_sim_66th_crop.png"), row = 5,  column = 1,scaling = "shrink", panel_clip = "on",label = "66th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5b_SEAS5_salmon_sim_75th_crop.png"), row = 6,  column = 1,scaling = "shrink", panel_clip = "on",label = "75th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5b_SEAS5_salmon_sim_95th_crop.png"), row = 7,  column = 1,scaling = "shrink", panel_clip = "on",label = "95th") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5b_SEAS5_salmon_sim_mean_crop.png"), row = 8,  column = 1,scaling = "shrink", panel_clip = "on",label = "mean") %>%
multipanelfigure::fill_panel(paste0(dirName,"Fig5b_legcrop.png"), row = 9,  column = 1,scaling = "shrink", panel_clip = "on",label = "")
Fig5a_ERA5
invisible(dev.off())
```

```{r figure-5b-salmon-plots-salmon-2, out.width="70%", eval = TRUE, warning=FALSE, fig.align = 'center', fig.cap = "(ref:figure-5b-salmon-plots-salmon-2)",echo = FALSE}
dirName <- paste0(system.file("extdata", package = "fishcastr"),
                  "/vignette_figures/Fig5b/salmon/")
knitr::include_graphics(path = paste0(dirName,"Fig5b_SEAS5.png"))
```

(ref:figure-5b-salmon-plots-salmon-2) *Tercile plots illustrating the model chain's propensity to correctly predict anomaly categories during the retrospective period (1993 - 2019) using ECMWF's SEAS5 as forcing data.*
