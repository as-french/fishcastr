...
# trout smolt count data in relation to photoperiod weighted degree days. The
...
data_stsmolt_forecast_years_water_temp.merge <- merge(data_water_temp,
                                                     data_stsmolt_forecast_years, by = "date")

data_stsmolt_forecast_years <- data_stsmolt_forecast_years_water_temp.merge

# WEIGHT TEMPERATURES (MULTIPLY TEMPS BY SCALED PHOTOPERIOD...) -------------------------------------
data_stsmolt_forecast_years$water_temp_weighted <- ifelse(data_stsmolt_forecast_years$water_temp <=0, 0, data_stsmolt_forecast_years$water_temp*(data_stsmolt_forecast_years$photoper/max(data_stsmolt_forecast_years$photoper)))
...
unique_forecast_years <- unique(data_stsmolt_forecast_years$salmonid_year)
...
for(i in 1:length(unique_forecast_years)){
  solstice_mat[i,"date_sols"] <- data_stsmolt_forecast_years$date[data_stsmolt_forecast_years$salmonid_year == unique_forecast_years[i]][which.min(data_stsmolt_forecast_years$photoper[data_stsmolt_forecast_years$salmonid_year == unique_forecast_years[i]])]
  solstice_mat[i,"forecast_year"] <- unique_forecast_years[i]
  solstice_mat[i,"date_end_year"] <- data_stsmolt_forecast_years$date[data_stsmolt_forecast_years$salmonid_year == unique_forecast_years[i]][which.max(data_stsmolt_forecast_years$salmonid_yday[data_stsmolt_forecast_years$salmonid_year == unique_forecast_years[i]])]
}
...
for(i in 1:length(unique_forecast_years)){
  list_salmonid_years[[i]] <- data_stsmolt_forecast_years[data_stsmolt_forecast_years$date >= as.Date(salmonid_list_data[i,"date_sols"]) & data_stsmolt_forecast_years$date <= as.Date(salmonid_list_data[i,"date_end_year"]),
...
# subset to 1981
data_stsmolt_dds.merge_1981 <- data_stsmolt_dds.merge[data_stsmolt_dds.merge$salmonid_year >= 1981,]

# convert counts to percentages of run
data_stsmolt_dds.merge_1981$stsmolt_perc_run <- NA

for(i in list(unique(data_stsmolt_dds.merge_1981[["salmonid_year"]]))[[1]]){
  data_stsmolt_dds.merge_1981$stsmolt_perc_run[data_stsmolt_dds.merge_1981[["salmonid_year"]] == i] <- 100*((data_stsmolt_dds.merge_1981[["stsmolt"]][data_stsmolt_dds.merge_1981[["salmonid_year"]] == i])/sum(data_stsmolt_dds.merge_1981[["stsmolt"]][data_stsmolt_dds.merge_1981[["salmonid_year"]] == i]))
}
# for(i in unique(data_stsmolt_dds.merge_1981$salmonid_year)){
#
#   plot(data_stsmolt_dds.merge_1981$weighted_dds_water[data_stsmolt_dds.merge_1981$stsmolt_perc_run > 0],
#      data_stsmolt_dds.merge_1981$stsmolt_perc_run[data_stsmolt_dds.merge_1981$stsmolt_perc_run > 0], cex = 0.5, col = "black", xlim = c(100,2500), ylim = c(0,35),main = i)
#
# points(data_stsmolt_dds.merge_1981$weighted_dds_water[data_stsmolt_dds.merge_1981$stsmolt_perc_run > 0 & data_stsmolt_dds.merge_1981$salmonid_year == i],
#      data_stsmolt_dds.merge_1981$stsmolt_perc_run[data_stsmolt_dds.merge_1981$stsmolt_perc_run > 0 & data_stsmolt_dds.merge_1981$salmonid_year == i], cex = 0.5, col = "green3")
# }
#legend("topright",
#       legend = unique(data_stsmolt_dds.merge_1981$salmonid_year),
#       col = unique(data_stsmolt_dds.merge_1981$salmonid_year),cex = 0.5, bty = "n",pch = 1)

# identify multiplication factor
mult_factor <- 0.5001/min(data_stsmolt_dds.merge_1981$stsmolt_perc_run[data_stsmolt_dds.merge_1981$stsmolt_perc_run > 0],na.rm = TRUE)
data_stsmolt_dds.merge_1981$stsmolt_perc_run_int <- round(data_stsmolt_dds.merge_1981$stsmolt_perc_run*mult_factor)
...
dataset_list <- lapply(data_list_years,FUN = function(x){
  data_stsmolt_dds.merge_1981[data_stsmolt_dds.merge_1981$salmonid_year %in% x,
                             c("stsmolt_perc_run_int","weighted_dds_water")]
})
...
# physio_negbin <-
#   EcoCountForecastR::physio_model_evaluation_exp_mod_Gauss(
...
#     file_path_plot = paste0(getwd(),'/vignettes/vignette_figures/FigS9/negbin_FigS9b.png'),
#     file_path_resid_fitted_plot = paste0(getwd(),'/vignettes/vignette_figures/FigS10/trout/negbin_FigS10b.png'),
#     file_path_dispersion_zeroinf_acf_plot = paste0(getwd(),'/vignettes/vignette_figures/FigS10/trout/negbin_FigS10c.png'),
#     file_path_coef_profile_plot = paste0(getwd(),'/vignettes/vignette_figures/FigS10/trout/negbin_FigS10d.png'),
#     start_values = list(c = 100, mu_exmg = 500, sigma_exmg = 50, lamb = 50, sigma_nb = 20),
#     lower_lims = list(c = 1, mu_exmg=200, sigma_exmg=1, lamb = 1, sigma_nb = 0.5),
#     ylims = c(-1200,1200),
#     xlims = c(0,1600)
#   )
#
...
model_physio_expmG_trout <- list("data" = physio_generalised_Poisson[["mod_physio_expmG"]]@data,
...
usethis::use_data(model_physio_expmG_trout, overwrite = TRUE)
