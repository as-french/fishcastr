# ----------------------------------------------------------------------------------- #
# A script for downloading operational year ECMWF ERA5 (here data through Jan
# 2020 to January 2021. The purpose of this script is to highlight the
# challenges in downloading ECMWF ERA5 data for operational years, where QA QC
# of ERA5 data leads to a more complex file format than simple retrospective
# forecast years.
#
# The ever-changing nature (in terms of QA QC) of near term data (e.g., most
# recent 3 months for ERA5) accessed through Copernicus CDS means this script is
# already obsolete. However, an important issue demonstrated by the code below
# (which was run during February 2021) is that variables differ in the nature of
# their QAQC. Here, to adjust for this, precipitation data and air temperature
# data had to be downloaded separately, paying close attention to time stamps at
# which quality was assured. Specifically, the issue dealt with here is the
# expver variable, which is coded 0001 for quality assured data and 0005 for
# non-assured data. As such, if data for 2020 Jan to Dec were downloaded in a
# single batch, the file format (as netcdf) is inconsistent with the format
# required for processing tools provided in the climate4R package.
#
# Ultimately, the implementation of an automatically generated "operational"
# forecast (as opposed to reforecast evaluation) relies on a consistent method
# for dealing with issues such as the below.

####################################################################################
# A NON R BASED SOLUTION
# if separate requests are not possible (no reason to think they wouldn't be) to
# get this file into correct format for using UNICAN tools DM used following code
# in command prompt cdo -f nc copy download.nc download_ok.nc this is based on
# CDO tools: https://code.mpimet.mpg.de/projects/cdo/ but there is an r wrapper:
# https://github.com/markpayneatwork/ClimateOperators alternative within r
# itself? or perhaps download a grib file from CDS instead
####################################################################################
...
request_Dec_1_t2m_0000_0600_2020 <- list(
  product_type = "reanalysis",
  format = "netcdf",
...
request_Dec_1_pr_0000_0600_2020 <- list(
  product_type = "reanalysis",
  format = "netcdf",
...
request_Dec_1_0700_end_month_2020 <- list(
  product_type = "reanalysis",
  format = "netcdf",
  variable = c("2m_temperature", "total_precipitation"),
  year = "2020",
...
request_Jan_1_0000_end_month_2021 <- list(
  product_type = "reanalysis",
  format = "netcdf",
  variable = c("2m_temperature", "total_precipitation"),
  year = "2021",
...
####################################################################################
# --------------------------------------------------------------------------- #
# Use the Climate4R tools to read in all downloaded nc files and bind to ERA5
# reforecast years raw data as first step in developing operational forecast for
# salmon 2021... The following code appends ERA5 data up to end of June 2019
# (which is the end of the antecedent conditions for the A. anguilla forecast)
# to the ERA5 downloads coded above (begninning July 2020 to end Jan 2021).
# --------------------------------------------------------------------------- #
...
# ------------------------------------------------------------------------------------ #
# Dec 2020 binding (separate for tas and pr)
# ------------------------------------------------------------------------------------ #
# t2m ----
variables <- c("t2m")
aggr.fun <- c("none")
...
# ------------------------------------------------------------------------------------- #
# A NOTE ON R PACKAGE VERSIONS FOR BIAS ADJUSTMENT
# ------------------------------------------------------------------------------------- #
# The computation of bias adjusted data depends on the R package version of the
# climate4R tools. Here, it is necessary to note that transformeR and downscaleR
# must be updated (or regressed to previous verions) together (see below).

# with downscaleR@v3.3.1 # Error in arr[, , , ind, , ] <- grid[["Data"]] :
...
# ------------------------------------------------------------------------------------- #
# also note that an na appears following bias correction
# which(is.na(data.bc.cross$pr$Data)) # row 9692 of pr is na (why?)
# don't use loo for operational year - it is automatic with
# "newdata" argument, but retain and append cross validated version for reforecast period to
