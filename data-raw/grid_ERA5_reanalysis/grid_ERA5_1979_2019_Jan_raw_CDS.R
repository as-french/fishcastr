...
#dir.data <- paste0(getwd(),'/inst/extdata/ECMWF_ERA5')
...
# subset to Jan 2019
grid_ERA5_subset_1979_2018 <- lapply(grid_ERA5_1979_2019_Jan_raw,
                                     function(x) transformeR::subsetGrid(x,
                                                      years = 1979:2018))
grid_ERA5_subset_jan_2019 <- lapply(grid_ERA5_1979_2019_Jan_raw,
                                    function(x) transformeR::subsetGrid(x,
                                                     years = 2019,
                                                     season = 1))
# bind grids
vars <- c("tas","pr","petH")
grid_ERA5_1979_2019_Jan_raw <- lapply(vars,function(x){
  transformeR::bindGrid(grid_ERA5_subset_1979_2018[[x]],
                        grid_ERA5_subset_jan_2019[[x]],
                        dimension = "time")
})
names(grid_ERA5_1979_2019_Jan_raw) <- vars

years <- range(lubridate::year(grid_ERA5_1979_2019_Jan_raw$tas$Dates$start))

saveRDS(grid_ERA5_1979_2019_Jan_raw,
        file = paste0(
          system.file("inst", package = "fishcastr"),
